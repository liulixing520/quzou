...
var kkpager = {
		...
		focus_gopage : function (){
			...
			btnGo.show();
			btnGo.css('left','0px');
			$('#'+this.gopageWrapId).css('border-color','#f83976');
			btnGo.animate({left: '+=58'}, 50,function(){
		...
		gopage : function(){
			...
			if(n >= this.total) n = this.total-1;
			if(this.mode == 'click'){
		...
		generPageHtml : function(config,enforceInit){
			...
			if(this.isShowFirstPageBtn){
				if(this.hasPrv){
					str_first = '<a '+this._getHandlerStr(0)+' title="'
			...
			if(this.isShowLastPageBtn){
				if(this.hasNext){
					str_last = '<a '+this._getHandlerStr(this.total-1)+' title="'
			...
			if(this.total <= 8){
				for(var i=0;i<this.total;i++){
					if(this.pno == i){
						str += '<span class="curr">'+parseInt(i+1)+'</span>';
					}else{
						str += '<a '+this._getHandlerStr(i)+' title="'
							+this.lang.buttonTipBeforeText + parseInt(i+1) + this.lang.buttonTipAfterText+'">'+parseInt(i+1)+'</a>';
					}
				}
			}else{
				if(this.pno <= 5){
					...
					str += '<a '+this._getHandlerStr(0)+' title="'
		...
		init : function(config){
			this.pno = isNaN(config.pno) ? 0 : parseInt(config.pno);
			...
			this.prv = (this.pno<2) ? 0 : (this.pno-1);
			this.next = (this.pno >= this.total-1) ? this.total : (this.pno + 1);
			this.hasPrv = (this.pno > 0);
			//此处是判断最后一个页码是否小于总页数
			this.hasNext = (this.pno+1 < this.total);
			this.inited = true;
		},
