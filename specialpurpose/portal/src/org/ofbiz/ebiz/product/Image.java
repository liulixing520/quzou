 ...
 * cropping, resizing, rotating, flipping� * Plus it let抯 you do some really
 * cool affects. * All is done super easily. * Combining operations can produce
 * some very cool results. * * Operations: * Open image. * Save image * Convert
 * image * Re-size image * Crop image * Convert to black and white image *
 * Rotate image * Flip image * Add color to image * Create image with multiple
 * instance of the original * Combining 2 images together * Emphasize parts of
 * the image * Affine transform image * * * @author Avi Yehuda *
 * 参考效果：http://www.aviyehuda.com/category/java/java-technologies/image-manipulation/
 */
...
public class Image {
	...
	/**
	 * Return image as java.awt.image.BufferedImage * @return image as
	...
	/**
	 * * Save the image as a new image file. * Can also convert the image
	 * according to file type. * @param fileName
	 * */
	...
	/**
	 * * Resizing the image by percentage of the original. * @param
	 * percentOfOriginal
	 * */
	...
	/**
	 * * Resizing the image by width and height. * @param newWidth * @param
	 * newHeight
	 * */
	public void resize(int newWidth, int newHeight) {
		...
		for (int y = 0; y < oldHeight && newY < newHeight; y++) {
			if (isNewImageHeightSmaller && heightCounter > 1) {
				// new image suppose to be smaller - skip row
				heightCounter -= 1;
			} else if (heightCounter < -1) {
				// new image suppose to be bigger - duplicate row
				heightCounter += 1;
				if (y > 1)
					y = y - 2;
				else
				...
				for (int x = 0; x < oldWidth && newX < newWidth; x++) {
					if (isNewImageWidthSmaller && widthCounter > 1) {
						// new image suppose to be smaller - skip column
						widthCounter -= 1;
					} else if (widthCounter < -1) {
						// new image suppose to be bigger - duplicate pixel
						widthCounter += 1;
						if (x > 1)
							x = x - 2;
						else
	...
	/**
	 * * Add color to the RGB of the pixel * @param numToAdd
	 * */
	...
	/**
	 * * Multiply the image. * @param timesToMultiplyVertically * @param
	...
	/**
	 * Multiply the image and also add color each of the multiplied images. * @param
	 * timesToMultiplyVertically * @param timesToMultiplyHorizantelly
	 * */
	...
	/**
	 * * Combines the image with another image in an equal presence to both; * @param
	 ...
	 * presence. If jump=3 than every 3rd * pixel is replaced by the new image.
	 * * As the jump is higher this is how much the new image has less presence.
	 * * @param newImagePath * @param jump
	 * */
	...
	public static void main(String[] args) {
		...
		Image image1 = new Image("c:/pics/israel_flag.gif");
		Image image2 = new Image("c:/pics/palestine_flag.gif");
		image1.resize(50);
		image2.resize(50);
		image1.affineTransform(0, 0.3);
		image2.affineTransform(0, -0.3);
		image1.combineWithPicture(image2);
		image1.saveAs("c:/pics/affineTransformAndCombine2.jpg");

		Image image3 = new Image("c:/pics/p1.jpg");
		image3.resize(50);
		image3.affineTransform(0.0, 0.5);
		image3.saveAs("c:/pics/affineTransform.jpg");

		Image image4 = new Image("c:/pics/heart.gif");
		image4.multiply(20,20);
		Image image5 = new Image("c:/pics/p6.jpg");
		image5.crop(800,0, -1, -1);
		image5.resize(50);
		image5.combineWithPicture(image, 3, Color.white);
		...
		Image image6 = new Image("c:/pics/p1.jpg");
		...
		Image image7 = new Image("c:/pics/p1.jpg");
		image.addColorToImage(Color.red, 5);
