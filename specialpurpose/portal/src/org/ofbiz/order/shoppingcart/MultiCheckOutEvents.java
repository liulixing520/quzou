 ...
 */
public class MultiCheckOutEvents {

	public static final String module = MultiCheckOutEvents.class.getName();
	public static final String resource = "OrderUiLabels";
	public static final String resource_error = "OrderErrorUiLabels";

	public static String cartNotEmpty(HttpServletRequest request, HttpServletResponse response) {
		MultiShoppingCartMap cartMap = MultiShoppingCartEvents.getCartMap(request);

		if (cartMap != null && cartMap.size() > 0) {
			return "success";
		} else {
			String errMsg = UtilProperties.getMessage(resource_error, "checkevents.cart_empty", (cartMap != null ? cartMap.getLocale() : UtilHttp.getLocale(request)));
	...
	public static String setPartialCheckOutOptions(HttpServletRequest request, HttpServletResponse response) {
		 ...
		 * request.setAttribute("_ERROR_MESSAGE_", null); return "success";
		 */
		return setCheckOutOptions(request, response);
	}
	...
	public static String createOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		String checkOutPaymentId = shoppingCartMap.getCheckOutPaymentId();
		if(checkOutPaymentId==null || checkOutPaymentId.equals("")){
			return "error";
		}
		List<ShoppingCart> shoppingCartList = shoppingCartMap.getShoppingCartList();
		...
		request.setAttribute("checkOutPaymentId", checkOutPaymentId);
		session.setAttribute("checkOutPaymentId", checkOutPaymentId);
		return "success";
	}
	...
	private static void calcTax(HttpServletRequest request) throws GeneralException {
		...
		Delegator delegator = (Delegator) request.getAttribute("delegator");
		// ShoppingCart cart = ShoppingCartEvents.getCartObject(request);
		MultiShoppingCartMap shoppingCartMap = (MultiShoppingCartMap) request.getSession().getAttribute("shoppingCartMap");
		List<ShoppingCart> shoppingCartList = shoppingCartMap.getShoppingCartList();
		for (ShoppingCart cart : shoppingCartList) {
			CheckOutHelper checkOutHelper = new CheckOutHelper(dispatcher, delegator, cart);
	...
	public static String determineNextFinalizeStep(HttpServletRequest request, HttpServletResponse response) {
		...
		// processing all of the changes, just to make sure we get everything...
	...
	private static BigDecimal determineBillingAccountAmount(String billingAccountId, String billingAccountAmount, LocalDispatcher dispatcher) {
		...
		// available balance or amount input if less than balance
		if (UtilValidate.isNotEmpty(billingAccountId)) {
	...
	public static String setShippingContactMechId(HttpServletRequest request, HttpServletResponse response) {
		try {
			MultiShoppingCartMap shoppingCartMap = MultiShoppingCartEvents.getCartMap(request);
			shoppingCartMap.setShippingContactMechId(request.getParameter("shippingContactMechId"));
		} catch (Exception e) {
			Debug.logError(e, module);
	...
	public static String setShippingMethod(HttpServletRequest request, HttpServletResponse response) {
		Delegator delegator = (Delegator) request.getAttribute("delegator");
		try {
			MultiShoppingCartMap shoppingCartMap = MultiShoppingCartEvents.getCartMap(request);
			...
			if (productStoreId == null) {
				request.setAttribute("info", "店铺标识丢失");
			}
			ShoppingCart cart = shoppingCartMap.getFromCartMap(productStoreId);
			if (shippingMethod == null || shippingMethod.trim().equals("")) {
				cart.setShipmentMethodTypeId(0, null);
				cart.setCarrierPartyId(0, null);
				cart.setProductStoreShipMethId(0, null);
				cart.setItemShipGroupEstimate(BigDecimal.ZERO, 0);
			} else {
				...
				List<GenericValue> productStoreShipMeths = delegator.findByAnd("ProductStoreShipmentMeth", UtilMisc.toMap("partyId", carrierPartyId, "shipmentMethodTypeId", shipmentMethodTypeId, "productStoreId", productStoreId), null, false);
				if (productStoreShipMeths != null && productStoreShipMeths.size() > 0) {
					cart.setProductStoreShipMethId(0, productStoreShipMeths.get(0).getString("productStoreShipMethId"));
				}
			}
		} catch (Exception e) {
			Debug.logError(e, module);
