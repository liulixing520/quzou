...
function subSearch(formName){
	var  searchType=document.getElementById('searchCanType').value;
	if(searchType!=null&&searchType.length>0){
		if(searchType=='orderId'){
			document.getElementById('orderIdCan').value=document.getElementById('searchCanContent').value;
		}else if(searchType=='userLoginId'){
			document.getElementById('userLoginIdCan').value=document.getElementById('searchCanContent').value;
		}
...
<div id='pageContent'  class="pageContent" >
	<div class="panelBar">
		<ul class="toolBar">
		<form action='FindCancelOrder' id='FindCancelOrder' name='FindCancelOrder' method="post" rel="pagerForm">
			  ...
			  <input type='hidden' name='orderId'  id='orderIdCan' />
			  <input type='hidden' name='userLoginId' id='userLoginIdCan' value='${requestParameters.userLoginId?if_exists}'/>
			...
			<li><select name='searchType' id='searchCanType'><option value='orderId'>订单号</option><option value='userLoginId'>下单人</option></select></li>
			<li><input type='text' name='searchContent' id='searchCanContent' onblur="if(this.value=='') this.value=''" onfocus="if(this.value=='') this.value=''" value="" ></li>
			<li><a class="search" href="#"  onclick="javascript:subSearch('FindCancelOrder');" ><span>搜索</span></a></li>
			<li><a class="search" href="<@ofbizUrl>FindCancelOrderAdvanced</@ofbizUrl>" target="dialog" mask="true" title="查询框"><span>高级检索</span></a></li>
		</form>	
			</ul>
	</div>
    ...
    <!--dwz  分页 -->      
	<@dwzPagin orderList![] orderListSize!0 VIEW_SIZE_1!20 VIEW_INDEX_1!0 "FindCancelOrder" "navTab"/>
</div>