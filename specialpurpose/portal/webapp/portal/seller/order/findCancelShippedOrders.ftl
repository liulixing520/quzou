...
function subSearch(formName){
	var  searchType=document.getElementById('searchCSHPType').value;
	if(searchType!=null&&searchType.length>0){
		if(searchType=='orderId'){
			document.getElementById('orderIdCSHP').value=document.getElementById('searchCSHPContent').value;
		}else if(searchType=='userLoginId'){
			document.getElementById('userLoginIdCSHP').value=document.getElementById('searchCSHPContent').value;
		}
...
<div id='pageContent'  class="pageContent" >
	<div class="panelBar">
		<ul class="toolBar">
		<form action='FindCancelShippedOrder' id='FindCancelShippedOrder' name='FindCancelShippedOrder' method="post" rel="pagerForm">
			  ...
			  <input type='hidden' name='orderId'  id='orderIdCSHP' />
			  <input type='hidden' name='userLoginId' id='userLoginIdCSHP' value='${requestParameters.userLoginId?if_exists}'/>
			  ...
			  <input type='hidden' name='shipmentStatusId' value='SHIPMENT_CANCELLED'/>
			...
			<li><select name='searchType' id='searchCSHPType'><option value='orderId'>订单号</option><option value='userLoginId'>下单人</option></select></li>
			<li><input type='text' name='searchContent' id='searchCSHPContent' onblur="if(this.value=='') this.value=''" onfocus="if(this.value=='') this.value=''" value="" ></li>
			<li><a class="search" href="#"  onclick="javascript:subSearch('FindCancelShippedOrder');" ><span>搜索</span></a></li>
			<li><a class="search" href="<@ofbizUrl>FindCancelShippedOrderAdvanced</@ofbizUrl>" target="dialog" mask="true" title="查询框"><span>高级检索</span></a></li>
		</form>	
			</ul>
	</div>
    ...
    <!--dwz  分页 -->      
	<@dwzPagin orderList![] orderListSize!0 VIEW_SIZE_1!20 VIEW_INDEX_1!0  "FindCancelShippedOrder" "navTab"/>
</div>