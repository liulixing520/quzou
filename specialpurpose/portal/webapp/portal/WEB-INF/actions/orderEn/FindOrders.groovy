...
if(parameters.orderId && parameters.statusId2){
	dispatcher.runSync("changeOrderStatus",[orderId:orderId,statusId:parameters.statusId2])
	/*order = delegator.findOne("OrderHeader", ["orderId" : parameters.orderId], false);
	if(order){
		order.put("statusId",parameters.statusId2);
		int update=delegator.store(order);
	}
	*/
	//println(order.orderDate.toString()+"*****************************************************************************");
}
if(parameters.orderId && parameters.ifRefuse && parameters.refuseReason){
	order = delegator.findOne("OrderHeader", ["orderId" : parameters.orderId], false);
	order.put("ifRefuse",parameters.ifRefuse);
	order.put("refuseReason",parameters.refuseReason);
	int update=delegator.store(order);
	//println(order.orderDate.toString()+"*****************************************************************************");
}
// get the order types
...
//VIEW_INDEX_1 = request.getParameter("VIEW_INDEX_1") ? Integer.valueOf(request.getParameter("VIEW_INDEX_1")) : 0;
//context.VIEW_INDEX_1 = VIEW_INDEX_1;

//VIEW_SIZE_1 = request.getParameter("VIEW_SIZE_1") ? Integer.valueOf(request.getParameter("VIEW_SIZE_1")) : 20;
//context.VIEW_SIZE_1 = VIEW_SIZE_1;


context.VIEW_INDEX_1 = request.getAttribute("VIEW_INDEX_1");
context.VIEW_SIZE_1 = request.getAttribute("VIEW_SIZE_1");
// get the lookup flag
...
context.viewSize   = request.getAttribute("VIEW_SIZE_1");
context.viewIndex    =  request.getAttribute("VIEW_INDEX_1");
