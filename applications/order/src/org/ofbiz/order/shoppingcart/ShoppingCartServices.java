...
public class ShoppingCartServices {
				...
				for (GenericValue item : orderItems) {
					...
					for (int g = 0; g < shipGroupAssocs.size(); g++) {
						...
						if (cartItem == null || cartItem.getQuantity() == null || BigDecimal.ZERO.equals(cartItem.getQuantity()) || shipGroupQty.equals(cartItem.getQuantity())) {
							Debug.logInfo("In loadCartFromOrder not adding item [" + item.getString("orderItemSeqId") + "] to ship group with index [" + itemIndex + "]; group quantity is [" + shipGroupQty + "] item quantity is [" + (cartItem != null ? cartItem.getQuantity() : "no cart item") + "] cartShipGroupIndex is [" + cartShipGroupIndex + "], csi.shipItemInfo.size(): " + csi.shipItemInfo.size(), module);
						} else {
	...
	public static Map<String, Object> loadCartFromQuote(DispatchContext dctx, Map<String, Object> context) {
		...
		// put them in a map: the key/values pairs are quoteItemSeqId/List of
		// adjs
		...
		if (UtilValidate.isNotEmpty(quoteItems)) {
			for (GenericValue quoteItem : quoteItems) {
				...
				if (quoteItem.get("productId") == null) {
					...
					try {
						// note that passing in null for itemGroupNumber as
						// there is no real grouping concept in the quotes right
						// now
		...
		if (applyQuoteAdjustments) {
			// The cart adjustments, derived from quote adjustments, are added
			// to the cart

			// Tax adjustments should be added to the shipping group and
			// shipping group item info
			// Other adjustments like promotional price should be added to the
			// cart independent of
			// the ship group.
			// We're creating the cart right now using data from the quote, so
			// there cannot yet be more than one ship group.
			...
			// The cart item adjustments, derived from quote item adjustments,
	...
	public static Map<String, Object> loadCartFromShoppingList(DispatchContext dctx, Map<String, Object> context) {
		...
		// If no currency has been set in the ShoppingList, use the ProductStore
		// default currency
		if (currency == null) {
			try {
		...
		// If we still have no currency, use the default from
		// general.properties. Failing that, use USD
		if (currency == null) {
		...
		if (UtilValidate.isNotEmpty(shoppingListItems)) {
			for (GenericValue shoppingListItem : shoppingListItems) {
				...
				/*
				 * BigDecimal amount =
				 * shoppingListItem.getBigDecimal("selectedAmount"); if (amount
