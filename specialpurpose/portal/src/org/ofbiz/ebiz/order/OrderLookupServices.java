...
public class OrderLookupServices {
    ...
    public static Map<String, Object> findOrders(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        Integer viewIndex = (Integer) context.get("VIEW_INDEX_1");
        Integer viewSize = (Integer) context.get("VIEW_SIZE_1");
        ...
        if (productStoreList != null) {
        	...
        	String selectChanneltype = (String) context.get("selectChanneltype");
        	if(UtilValidate.isNotEmpty(selectChanneltype)){
        		if("POS_CHANNEL".equals(selectChanneltype)){
        			//判断登陆用户管理的店铺
        			try {
						List<GenericValue> productStoresList = EntityUtil.filterByDate(delegator.findByAnd("ProductStoreRole", UtilMisc.toMap("partyId", userLogin.getString("partyId"))));
						if(UtilValidate.isNotEmpty(productStoresList)){
							List<String> psIds = EntityUtil.getFieldListFromEntityList(productStoresList, "productStoreId", true);
							conditions.add(EntityCondition.makeCondition("productStoreId", EntityOperator.IN, psIds));
						}else{
							conditions.add(EntityCondition.makeCondition("productStoreId", EntityOperator.EQUALS, ""));
						}
        		...
        		conditions.add(EntityCondition.makeCondition("productStoreId", EntityOperator.EQUALS, ProductStoreWorkerExt.getPartyCurrentStoreId(delegator, userLogin.getString("partyId"))));
        	}
        }
        ...
        String estimatedShipFacilityId = (String) context.get("estimatedShipFacilityId");
        if (UtilValidate.isNotEmpty(estimatedShipFacilityId)) {
            paramList.add("estimatedShipFacilityId=" + estimatedShipFacilityId);
            conditions.add(makeExpr("estimatedShipFacilityId", estimatedShipFacilityId));
        }
        ...
        String finAccountTypeId= (String)context.get("finAccountTypeId");  //add by hongs on 2012-09-12
        String paymentMethodTypeId = (String)context.get("paymentMethodTypeId");
        String oppStatusId = (String)context.get("oppStatusId");
        
        if (finAccountId != null || cardNumber != null || accountNumber != null || finAccountTypeId !=null || paymentMethodTypeId!=null || oppStatusId!=null) {
            dve.addMemberEntity("OP", "OrderPaymentPreference");
            dve.addAlias("OP", "finAccountId");
            dve.addAlias("OP", "paymentMethodId");
            dve.addAlias("OP", "paymentMethodTypeId");
            dve.addAlias("OP", "oppStatusId","statusId",null,null,null,null);
        ...
        if (UtilValidate.isNotEmpty(finAccountTypeId)) { //会员卡、提货卡 2012-09-12
            dve.addMemberEntity("FA", "FinAccount");
            dve.addAlias("FA", "finAccountTypeId");
            dve.addViewLink("OP", "FA", Boolean.FALSE, UtilMisc.toList(new ModelKeyMap("finAccountId", "finAccountId")));

            paramList.add("finAccountTypeId=" + finAccountTypeId);
            conditions.add(makeExpr("finAccountTypeId", finAccountTypeId));
        }
        
        if (UtilValidate.isNotEmpty(oppStatusId)) {
            paramList.add("oppStatusId=" + oppStatusId);
            conditions.add(makeExpr("paymentStatus", oppStatusId));
        }
        
        if (UtilValidate.isNotEmpty(paymentMethodTypeId)) {
            paramList.add("paymentMethodTypeId=" + paymentMethodTypeId);
            conditions.add(makeExpr("paymentMethodTypeId", paymentMethodTypeId));
        }
        ...
        if (UtilValidate.isNotEmpty(shipmentStatusId)) {
            dve.addMemberEntity("SHIPMENT", "Shipment");
            dve.addAlias("SHIPMENT", "shipmentStatusId","statusId",null,null,null,null);
            dve.addViewLink("OH", "SHIPMENT", Boolean.TRUE, UtilMisc.toList(new ModelKeyMap("orderId", "primaryOrderId")));
           
            paramList.add("shipmentStatusId=" + shipmentStatusId);
            conditions.add(makeExpr("shipmentStatusId", shipmentStatusId));
        }
        ...
        if (UtilValidate.isNotEmpty(countryGeoId) && UtilValidate.isNotEmpty(includeCountry) ||UtilValidate.isNotEmpty(stateGeoId) || UtilValidate.isNotEmpty(cityGeoId) ||UtilValidate.isNotEmpty(countyGeoId)) {
            ...
            dve.addAlias("PA", "stateProvinceGeoId");
            dve.addAlias("PA", "cityGeoId");
            dve.addAlias("PA", "countyGeoId");
            ...
            if (UtilValidate.isNotEmpty(stateGeoId) || UtilValidate.isNotEmpty(cityGeoId) ||UtilValidate.isNotEmpty(countyGeoId)) {
            	List<EntityCondition> exprs = new LinkedList<EntityCondition>();
            	exprs.add(EntityCondition.makeCondition("contactMechPurposeTypeId", EntityOperator.EQUALS, "SHIPPING_LOCATION"));
            	if (UtilValidate.isNotEmpty(stateGeoId)) {
            		paramList.add("stateGeoId=" + stateGeoId);
            		exprs.add(EntityCondition.makeCondition("stateProvinceGeoId", EntityOperator.EQUALS, stateGeoId));
            	}
            	if (UtilValidate.isNotEmpty(cityGeoId)) {
            		paramList.add("cityGeoId=" + cityGeoId);
            		exprs.add(EntityCondition.makeCondition("cityGeoId", EntityOperator.EQUALS, cityGeoId));
            	}
            	if (UtilValidate.isNotEmpty(countyGeoId)) {
            		paramList.add("countyGeoId=" + countyGeoId);
            		exprs.add(EntityCondition.makeCondition("countyGeoId", EntityOperator.EQUALS, countyGeoId));
            	}
        ...
        result.put("VIEW_INDEX_1", viewIndex);
        result.put("VIEW_SIZE_1", viewSize);
        result.put("aotuSearch", context.get("aotuSearch"));
