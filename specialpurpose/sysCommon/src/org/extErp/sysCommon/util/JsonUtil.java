...
public class JsonUtil
{
    ...
    /**
     * 将json解成map或者list
     * @param rows json数据（[{dateTime:[{aa:'a2',bb:{1:11,2:22,3:'[111]'}},{cc:'1c',dd:'1d'}],dayType:'dayType_1'},{dateTime:'2014-11-12',dayType:'dayType_2'}]）
     * @return
     */
    public static Object toMapByRowsTest(String rows){
    	if(UtilValidate.isNotEmpty(rows)){
    		if(rows.substring(0,1).equals("[")){//list
    			List list = new ArrayList();
				JSONArray oneJsonArray = JSONArray.fromObject(rows);
				if (UtilValidate.isNotEmpty(oneJsonArray)) {
					for (Object object : oneJsonArray) {
						list.add(toMapByRowsTest(object.toString()));
					}
				}
    			return list;
    		}else if(rows.substring(0,1).equals("{")){//map.
    			Map map = new HashMap();
				JSONObject oneJsonObject = JSONObject.fromObject(rows);
				if(UtilValidate.isNotEmpty(oneJsonObject)){
					for (Object obj : oneJsonObject.keySet()) {
						String mapValue = String.valueOf(oneJsonObject.get(obj));
						map.put(obj, toMapByRowsTest(mapValue));
					}
				}
    			return map;
    		}
    ...
    public static Object toMapByRowsTestTwo(String rows){
    	if(UtilValidate.isNotEmpty(rows)){
    		if(rows.substring(0,1).equals("[")){//list
    			List list = JSONArray.fromObject(rows);
    			return list;
    		}else if(rows.substring(0,1).equals("{")){//map.
    			Map map = JSONObject.fromObject(rows);
    			return map;
    		}
    		return 	rows;
    	}
