...
/**
 * <p>
 * <b>Title:</b> ShoppingCartItem.java
 * <p>
 * <b>Description:</b> Shopping cart item object.
 */
...
public class ShoppingCartItem implements java.io.Serializable {
	...
	// The following three optional fields are used to collect information for
	// the OrderItemAssoc entity
	private String associatedOrderId = null; // the order Id, if any, to which
												// the given item is associated
												// (typically a sales order item
												// can be associated to a
												// purchase order item, for
												// example in drop shipments)
	private String associatedOrderItemSeqId = null; // the order item Id, if
													// any, to which the given
													// item is associated
	private String orderItemAssocTypeId = "PURCHASE_ORDER"; // the type of
															// association
															// between this item
															// and an external
															// item; by default,
	...
	/**
	 * Makes a ShoppingCartItem for a purchase order item and adds it to the
	 * cart. NOTE: This method will get the product entity and check to make
	 * sure it can be purchased.
	 * 
	 * @param cartLocation
	 *            The location to place this item; null will place at the end
	 * @param productId
	 *            The primary key of the product being added
	 * @param quantity
	 *            The quantity to add
	 * @param additionalProductFeatureAndAppls
	 *            Product feature/appls map
	 * @param attributes
	 *            All unique attributes for this item (NOT features)
	 * @param prodCatalogId
	 *            The catalog this item was added from
	 * @param configWrapper
	 *            The product configuration wrapper (null if the product is not
	 *            configurable)
	 * @param dispatcher
	 *            LocalDispatcher object for doing promotions, etc
	 * @param cart
	 *            The parent shopping cart object this item will belong to
	 * @param supplierProduct
	 *            GenericValue of SupplierProduct entity, containing product
	 ...
	 * @param cancelBackOrderDate
	 *            The date which if crossed causes order cancellation
	...
	public static ShoppingCartItem makePurchaseOrderItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher, ShoppingCart cart, GenericValue supplierProduct, Timestamp shipBeforeDate, Timestamp shipAfterDate, Timestamp cancelBackOrderDate) throws CartItemModifyException, ItemNotFoundException {
		...
		// set the ship before/after/dates and cancel back order date. this
		// needs to happen before setQuantity because setQuantity causes the
		// ship group's dates to be
	...
	/**
	 * Makes a ShoppingCartItem and adds it to the cart. NOTE: This method will
	 * get the product entity and check to make sure it can be purchased.
	 * 
	 * @param cartLocation
	 *            The location to place this item; null will place at the end
	 * @param productId
	 ...
	 *            Optional. Defaults to 0.0. If a selectedAmount is needed
	 *            (complements the quantity value), pass it in here.
	 * @param quantity
	 *            Required. The quantity to add.
	 * @param unitPrice
	 *            Optional. Defaults to 0.0, which causes calculation of price.
	 * @param reservStart
	 *            Optional. The start of the reservation.
	 * @param reservLength
	 *            Optional. The length of the reservation.
	 * @param reservPersons
	 *            Optional. The number of persons taking advantage of the
	 *            reservation.
	 * @param shipBeforeDate
	 *            Optional. The date to ship the order by.
	 * @param shipAfterDate
	 *            Optional. Wait until this date to ship.
	 * @param additionalProductFeatureAndAppls
	 *            Optional. Product feature/appls map.
	 * @param attributes
	 *            Optional. All unique attributes for this item (NOT features).
	 * @param prodCatalogId
	 *            Optional, but strongly recommended. The catalog this item was
	 *            added from.
	 * @param configWrapper
	 *            Optional. The product configuration wrapper (null if the
	 *            product is not configurable).
	 * @param itemType
	 *            Optional. Specifies the type of cart item, corresponds to an
	 *            OrderItemType and should be a valid orderItemTypeId.
	 * @param itemGroup
	 *            Optional. Specifies which item group in the cart this should
	 *            belong to, if item groups are needed/desired.
	 * @param dispatcher
	 *            Required (for price calculation, promos, etc). LocalDispatcher
	 *            object for doing promotions, etc.
	 * @param cart
	 *            Required. The parent shopping cart object this item will
	 *            belong to.
	 * @param triggerExternalOpsBool
	 *            Optional. Defaults to true. Trigger external operations (like
	 *            promotions and such)?
	 * @param triggerPriceRulesBool
	 *            Optional. Defaults to true. Trigger the price rules to
	 *            calculate the price for this item?
	 * 
	 * @return a new ShoppingCartItem object
	 * @throws CartItemModifyException
	 */
	public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, Timestamp shipBeforeDate, Timestamp shipAfterDate, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher, ShoppingCart cart, Boolean triggerExternalOpsBool,
			Boolean triggerPriceRulesBool, String parentProductId, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException, ItemNotFoundException {
	...
	public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId, String accommodationSpotId, Timestamp shipBeforeDate, Timestamp shipAfterDate, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher,
			ShoppingCart cart, Boolean triggerExternalOpsBool, Boolean triggerPriceRulesBool, String parentProductId, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException, ItemNotFoundException {
		Delegator delegator = cart.getDelegator();
	...
	/**
	 * Makes a ShoppingCartItem and adds it to the cart. WARNING: This method
	 * does not check if the product is in a purchase category. rental fields
	 * were added.
	 * 
	 * @param cartLocation
	 *            The location to place this item; null will place at the end
	...
	public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, Timestamp shipBeforeDate, Timestamp shipAfterDate, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher, ShoppingCart cart, Boolean triggerExternalOpsBool,
			Boolean triggerPriceRulesBool, GenericValue parentProduct, Boolean skipInventoryChecks, Boolean skipProductChecks) throws CartItemModifyException {
	...
	public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId, String accommodationSpotId, Timestamp shipBeforeDate, Timestamp shipAfterDate, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher,
		...
		// set the ship before and after dates (defaults to cart ship
		// before/after dates)
		newItem.setShipBeforeDate(shipBeforeDate != null ? shipBeforeDate : cart.getDefaultShipBeforeDate());
		newItem.setShipAfterDate(shipAfterDate != null ? shipAfterDate : cart.getDefaultShipAfterDate());
	...
	public static void isValidCartProduct(ProductConfigWrapper configWrapper, GenericValue product, Timestamp nowTimestamp, Locale locale) throws CartItemModifyException {
		 ...
		 * "Y".equals(product.getString("salesDiscWhenNotAvail"))) { // check
		 * atp and if <= 0 then the product is no more available because // all
		 * the units in warehouse are reserved by other sales orders and no new
		 * purchase orders will be done // for this product. if
		 * (!newItem.isInventoryAvailableOrNotRequired(quantity,
		 * cart.getProductStoreId(), dispatcher)) { Map messageMap =
		 * UtilMisc.toMap("productName", product.getString("productName"),
		 * "productId", product.getString("productId"));
		 * 
		 * String excMsg = UtilProperties.getMessage(resource_error,
		 * "item.cannot_add_product_no_longer_available", messageMap , locale);
		 * 
		 * Debug.logWarning(excMsg, module); throw new
		 * CartItemModifyException(excMsg); } }
		 */
	...
	/**
	 * Makes a non-product ShoppingCartItem and adds it to the cart. NOTE: This
	 * is only for non-product items; items without a product entity (work
	 * items, bulk items, etc)
	 * 
	 * @param cartLocation
	 *            The location to place this item; null will place at the end
	 * @param itemType
	 *            The OrderItemTypeId for the item being added
	 * @param itemDescription
	 *            The optional description of the item
	 * @param productCategoryId
	 *            The optional category the product *will* go in
	 * @param basePrice
	 ...
	 *            The parent shopping cart object this item will belong to
	 * @param triggerExternalOpsBool
	 *            Indicates if we should run external operations (promotions,
	 *            auto-save, etc)
	 * @return a new ShoppingCartItem object
	 * @throws CartItemModifyException
	 */
	...
	protected ShoppingCartItem(GenericValue product, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, Locale locale, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup) {
		...
		if (product != null) {
			...
			// associated virtual product and get the productName of that
			// product
	...
	protected ShoppingCartItem(GenericValue product, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, String prodCatalogId, ProductConfigWrapper configWrapper, Locale locale, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, GenericValue parentProduct) {
		...
		if (UtilValidate.isEmpty(itemType)) {
			if (UtilValidate.isNotEmpty(_product.getString("productTypeId"))) {
				...
				// NOTE DEJ20100111: it seems safe to assume here that because a
				// product is passed in that even if the product has no type
				// this type of item still applies; thanks to whoever wrote the
				// previous code, that's a couple of hours tracking this down
				// that I wouldn't have minded doing something else with... :)
				this.itemType = "PRODUCT_ORDER_ITEM";
			}
		} else {
		...
		if (UtilValidate.isNotEmpty(configWrapper)) {
			...
			if (UtilValidate.isEmpty(configWrapper.getConfigId())) { // new
																		// product
																		// configuration.
																		// Persist
																		// it
	...
	/**
	 * returns "OK" when the product can be booked or returns a string with the
	 * dates the related fixed Asset is not available
	 */
	public static String checkAvailability(String productId, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, ShoppingCart cart) {
		...
		// see if this fixed asset has a calendar, when no create one and attach
		// to fixed asset
		// DEJ20050725 this isn't being used anywhere, commenting out for now
		// and not assigning from the getRelatedOne: GenericValue
		// techDataCalendar = null;
		...
		// please note that calendarId is the same for (TechData)Calendar,
		// CalendarExcDay and CalendarExWeek
		long dayCount = 0;
		...
		while (BigDecimal.valueOf(dayCount).compareTo(reservLength) < 0) {
			...
			try {
				techDataCalendarExcDay = delegator.findOne("TechDataCalendarExcDay", UtilMisc.toMap("calendarId", fixedAsset.get("calendarId"), "exceptionDateStartTime", exceptionDateStartTime), false);
			} catch (GenericEntityException e) {
				Debug.logWarning(e, module);
			}
			if (techDataCalendarExcDay == null) {
				...
				// first try techDataCalendarExcDay(exceptionCapacity) and then
				// FixedAsset(productionCapacity)
	...
	protected void setQuantity(BigDecimal quantity, LocalDispatcher dispatcher, ShoppingCart cart, boolean triggerExternalOps, boolean resetShipGroup, boolean updateProductPrice, boolean skipInventoryChecks) throws CartItemModifyException {
		...
		if (!skipInventoryChecks && !"PURCHASE_ORDER".equals(cart.getOrderType())) {
			...
			// don't worry about inventory getting pulled out from under, that
			// will be handled at checkout time
		...
		if (resetShipGroup) {
			 ...
			 * 
			 * int shipGroupIndex = -1; if
			 * ("PURCHASE_ORDER".equals(cart.getOrderType())) { shipGroupIndex =
			 * 0; } else { if (_product != null &&
			 * "PRODRQM_DS".equals(_product.getString
			 * ("requirementMethodEnumId"))) { // this is a drop-ship only
			 * product: we need a ship group with supplierPartyId set Map
			 * supplierProductsResult = null; try { supplierProductsResult =
			 * dispatcher.runSync("getSuppliersForProduct",
			 * UtilMisc.toMap("productId", _product.getString("productId"),
			 * "quantity", Double.valueOf(quantity), "currencyUomId",
			 * cart.getCurrency(), "canDropShip", "Y", "userLogin",
			 * cart.getUserLogin())); List productSuppliers =
			 * (List)supplierProductsResult.get("supplierProducts");
			 * GenericValue supplierProduct =
			 * EntityUtil.getFirst(productSuppliers); if (supplierProduct !=
			 * null) { String supplierPartyId =
			 * supplierProduct.getString("partyId"); List shipGroups =
			 * cart.getShipGroups(); for(int i = 0; i < shipGroups.size(); i++)
			 * { ShoppingCart.CartShipInfo csi =
			 * (ShoppingCart.CartShipInfo)shipGroups.get(i); if
			 * (supplierPartyId.equals(csi.getSupplierPartyId())) {
			 * shipGroupIndex = i; break; } } if (shipGroupIndex == -1) { //
			 * create a new ship group shipGroupIndex = cart.addShipInfo();
			 * cart.setSupplierPartyId(shipGroupIndex, supplierPartyId); } } }
			 * catch (Exception e) { Debug.logWarning(
			 * "Error calling getSuppliersForProduct service, result is: " +
			 * supplierProductsResult, module); } }
			 * 
			 * if (shipGroupIndex == -1) { List shipGroups =
			 * cart.getShipGroups(); for(int i = 0; i < shipGroups.size(); i++)
			 * { ShoppingCart.CartShipInfo csi =
			 * (ShoppingCart.CartShipInfo)shipGroups.get(i); if
			 * (csi.getSupplierPartyId() == null) { shipGroupIndex = i; break; }
			 * } if (shipGroupIndex == -1) { // create a new ship group
		...
		if (_product != null && isModifiedPrice == false) {
			try {
				...
				if (cart.getOrderType().equals("PURCHASE_ORDER")) {
					...
					// check if a survey is associated with the item and add to
					...
					if (isAlternativePacking) {
						...
						if (priceResult.get("price") != null) {
							//add by wangyg 代销价格 
							if(UtilValidate.isNotEmpty(cart.getDxStoreId())){
								try {
									GenericValue gv=delegator.findOne("DxProduct", false, 
											UtilMisc.toMap("productId",productId,"dxStoreId",cart.getDxStoreId(),"supplierStoreId",cart.getProductStoreId()));
									if(UtilValidate.isNotEmpty(gv)){
										this.setDisplayPrice(((BigDecimal) gv.get("dxPrice")).divide(pieces, decimals, RoundingMode.HALF_UP));
										this.setBasePrice(((BigDecimal)  gv.get("dxPrice")).divide(pieces, decimals, RoundingMode.HALF_UP));
									}
								} catch (GenericEntityException e) {
							...
							if(UtilValidate.isNotEmpty(cart.getDxStoreId())){
								try {
									GenericValue gv=delegator.findOne("DxProduct", false, 
											UtilMisc.toMap("productId",productId,"dxStoreId",cart.getDxStoreId(),"supplierStoreId",cart.getProductStoreId()));
									if(UtilValidate.isNotEmpty(gv)){
										this.setDisplayPrice((BigDecimal) gv.get("dxPrice"));
										this.setBasePrice(((BigDecimal) gv.get("dxPrice")));
									}
								} catch (GenericEntityException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
					...
					if (configWrapper != null) {
						// TODO: for configurable products need to do something
						// to make them VAT aware... for now base and display
						// prices are the same
					...
					// no try to do a recurring price calculation; not all
					// products have recurring prices so may be null
					...
					// for the recurring price only set the values iff
					// validPriceFound is true
	...
	public synchronized BigDecimal addPromoQuantityCandidateUse(BigDecimal quantityDesired, GenericValue productPromoCondAction, boolean checkAvailableOnly) {
		...
		if (promoQuantityAvailable.compareTo(BigDecimal.ZERO) > 0) {
			...
			if (!checkAvailableOnly) {
				...
				// ", for promoCondAction: " + productPromoCondAction, module);
	...
	public String getDescription() {
		...
		if (product != null) {
			...
			// associated virtual product and get the description of that
			// product
	...
	public BigDecimal getWeight() {
		GenericValue product = getProduct();
		if (product != null) {
			//@WuHK portal用productWeight表示重量
			//BigDecimal weight = product.getBigDecimal("weight");
			BigDecimal weight = product.getBigDecimal("productWeight");
			// if the weight is null, see if there is an associated virtual
			// product and get the weight of that product
			if (weight == null) {
				GenericValue parentProduct = this.getParentProduct();
				if (parentProduct != null)
	...
	public long getPiecesIncluded() {
		...
		if (product != null) {
			...
			// if the piecesIncluded is null, see if there is an associated
			// virtual product and get the piecesIncluded of that product
			if (pieces == null) {
				GenericValue parentProduct = this.getParentProduct();
				if (parentProduct != null)
	...
	public BigDecimal getSize() {
		...
		if (product != null) {
			...
			// if all are null, see if there is an associated virtual product
			// and get the info of that product
	...
	/**
	 * calculates for a reservation the percentage/100 extra for more than 1
	 * person.
	 */
	...
	public BigDecimal getRentalAdjustment() {
		...
		rentalValue = rentalValue.add(new BigDecimal("100")); // add final 100
																// percent for
																// first person
		// Debug.logInfo("rental parameters....Nbr of persons:" +
		// getReservPersons() + " extra% 2nd person:" + getReserv2ndPPPerc()+
		// " extra% Nth person:" + getReservNthPPPerc() +
		// "  total rental adjustment:" + rentalValue/100 * getReservLength());
		return rentalValue.movePointLeft(2).multiply(getReservLength()); // return
																			// total
																			// rental
																			// adjustment
	}
	...
	public BigDecimal getItemSubTotal(BigDecimal quantity) {
		...
		// " Rental adj:" + getRentalAdjustment() + " other adj:" +
	...
	public void putAdditionalProductFeatureAndAppl(GenericValue additionalProductFeatureAndAppl) {
		...
		// if one already exists with the given type, remove it with the
		...
		// NOTE: this is a VERY simple pricing scheme for additional features
		// and will likely need to be extended for most real applications
	...
	/**
	 * Add an adjustment to the order item; don't worry about setting the
	 * orderId, orderItemSeqId or orderAdjustmentId; they will be set when the
	 * order is created
	 */
	...
	/**
	 * Remove the contactMechId from this item given the
	...
	/**
	 * Compares the specified object with this cart item. Defaults isPromo to
	 * false. Default to no itemGroup.
	 */
	...
	public boolean equals(String productId, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons, String accommodationMapId, String accommodationSpotId, Map<String, GenericValue> additionalProductFeatureAndAppls, Map<String, Object> attributes, Map<String, String> orderItemAttributes, String prodCatalogId, BigDecimal selectedAmount, ProductConfigWrapper configWrapper, String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, boolean isPromo) {
		...
		if ((this.additionalProductFeatureAndAppls == null && UtilValidate.isNotEmpty(additionalProductFeatureAndAppls)) || (UtilValidate.isNotEmpty(this.additionalProductFeatureAndAppls) && additionalProductFeatureAndAppls == null) || (this.additionalProductFeatureAndAppls != null && additionalProductFeatureAndAppls != null && (this.additionalProductFeatureAndAppls.size() != additionalProductFeatureAndAppls.size() || !(this.additionalProductFeatureAndAppls.equals(additionalProductFeatureAndAppls))))) {
			return false;
		}

		if ((this.attributes == null && UtilValidate.isNotEmpty(attributes)) || (UtilValidate.isNotEmpty(this.attributes) && attributes == null) || (this.attributes != null && attributes != null && (this.attributes.size() != attributes.size() || !(this.attributes.equals(attributes))))) {
			return false;
		}
	...
	/**
	 * Gets the Product entity. If it is not already retreived gets it from the
	 * delegator
	 */
