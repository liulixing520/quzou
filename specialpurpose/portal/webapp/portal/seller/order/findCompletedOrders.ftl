...
function subSearch(formName){
	var  searchType=document.getElementById('searchCPOType').value;
	if(searchType!=null&&searchType.length>0){
		if(searchType=='orderId'){
			document.getElementById('orderIdCPO').value=document.getElementById('searchCPOContent').value;
		}else if(searchType=='userLoginId'){
			document.getElementById('userLoginIdCPO').value=document.getElementById('searchCPOContent').value;
		}
...
<div id='pageContent'  class="pageContent" >
	<div class="panelBar">
		<ul class="toolBar">
		<form action='FindCompletedOrder' id='FindCompletedOrder' name='FindCompletedOrder' method="post" rel="pagerForm">
			  ...
			  <input type='hidden' name='orderId'  id='orderIdCPO' />
			  <input type='hidden' name='userLoginId' id='userLoginIdCPO' value='${requestParameters.userLoginId?if_exists}'/>
			  ...
			  <input type='hidden' name='shipmentStatusId' value='SHIPMENT_DELIVERED'/>
			...
			<li><select name='searchType' id='searchCPOType'><option value='orderId'>订单号</option><option value='userLoginId'>下单人</option></select></li>
			<li><input type='text' name='searchContent' id='searchCPOContent' onblur="if(this.value=='') this.value=''" onfocus="if(this.value=='') this.value=''" value="" ></li>
			<li><a class="search" href="#"  onclick="javascript:subSearch('FindCompletedOrder');" ><span>搜索</span></a></li>
			<li><a class="search" href="<@ofbizUrl>FindCompletedOrderAdvanced</@ofbizUrl>" target="dialog" mask="true" title="查询框"><span>高级检索</span></a></li>
		</form>	
			</ul>
	</div>
    ...
    <!--dwz  分页 -->      
	<@dwzPagin orderList![] orderListSize!0 VIEW_SIZE_1!20 VIEW_INDEX_1!0  "FindCompletedOrder" "navTab"/>
</div>