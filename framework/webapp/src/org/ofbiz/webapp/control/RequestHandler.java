...
public class RequestHandler {
    ...
    public void doRequest(HttpServletRequest request, HttpServletResponse response, String chain,
        ...
        if ("request".equals(nextRequestResponse.type)) {
            ...
            } else if ("request-redirect-allparam".equals(nextRequestResponse.type))
    	    {// add by wangyg
        		if (Debug.verboseOn())
        		    Debug.logVerbose("[RequestHandler.doRequest]: Response is a Request redirect with no parameters." + " sessionId=" + UtilHttp.getSessionId(request), module);
        		callRedirectNoParam(makeLink(request, response, nextRequestResponse.value, false, false, true), response, request);
        	    } else if ("request-redirect-param".equals(nextRequestResponse.type))
        	    {
        	    	if (Debug.verboseOn())
        	    		Debug.logVerbose("[RequestHandler.doRequest]: Response is a Request redirect with no parameters." + " sessionId=" + UtilHttp.getSessionId(request), module);
        	    	String url = makeLink(request, response, nextRequestResponse.value, false, false, true)+this.makeQueryStringBaseRequestAttribute(request, nextRequestResponse);;
        	    	callRedirectNoParam(url, response, request);
 ...
 // add by wangyg
    private void callRedirectNoParam(String url, HttpServletResponse resp, HttpServletRequest req) throws RequestHandlerException
	    {
		if (Debug.infoOn())
		    Debug.logInfo("Sending redirect to: [" + url + "], sessionId=" + UtilHttp.getSessionId(req), module);
		// set the attributes in the session so we can access it.
		Enumeration<String> attributeNameEnum = UtilGenerics.cast(req.getAttributeNames());
		...
		while (attributeNameEnum.hasMoreElements())
		{
		    ...
		    if (obj instanceof Serializable)
		    {
			// reqAttrMap.put(name, obj); //注视wangyg
		    }
		}
		if (reqAttrMap.size() > 0)
		{
    ...
    /**
     * 将redirect-parameter中设置的属性值返回
     * @param request
     * @param requestResponse
     * @return
     */
    public String makeQueryStringBaseRequestAttribute(HttpServletRequest request, ConfigXMLReader.RequestResponse requestResponse) {
