...
<table width="100%" border="1" cellpadding="0" cellspacing="0"
	...
	$(function(){
		...
		-->
		$(".breadcrumb").html("<li><i class='ace-icon fa fa-home home-icon'></i><a href='/portal/control/sellerIndex'>首页</a></li><li class='active'>报表管理</li><li class='active'>我的买家列表</li>")
         ...
         <div class="page-content" style="border:1px solid #ccc;margin:15px; margin-top:5px;"> 
          <div class="page-content-area"> 
           <div class="page-header" style="padding-bottom:5px; height:50px;"> 
            <h1 style="padding-top:15px; font-size:18px; float:left;">我的买家列表</h1>
            <a class="smallSubmit" style="float:right; margin-top:5px;" href="<@ofbizUrl>MySellerBuyerReport.xls</@ofbizUrl>">导出EXCEL</a> 
           </div>
           <div class="row" style=""> 
            <div class="col-xs-12"> 
              ...
              <table cellspacing="0" class="basic-table hover-bar"> 
               <tbody> 
                <tr class="header-row-2"> 
                 ...
                 <td><a class="sort-order" href="">登录名</a></td>
                 <td><a class="sort-order" href="">地址</a></td> 
                 <td><a class="sort-order" href="">邮编</a></td>
                 <#--  
                 <td><a class="sort-order" href="">第二金额</a></td> 
 				-->
                </tr>
                <#if mySellerBuyerList?has_content>
                	<#list mySellerBuyerList as party>
                	
                	<tr> 
                 		<td class="center"><label class="position-relative"> <input type="checkbox" class="ace" /> <span class="lbl"></span> </label> </td> 
                 		<td>${party.attnName!}</td> 
                 		<td class="hidden-480">${party.createdBy!}</td>
                 		<#--  
                 		<#if party.statusId?has_content && party.statusId=="ORDER_COMPLETED">
                 			<td>已完成</td>
                 		</#if>
                 		<#if party.statusId?has_content && party.statusId=="ORDER_APPROVED">
                 			<td>待发货</td>
                 		</#if>
                 		<#if party.statusId?has_content && party.statusId=="ORDER_SENT">
                 			<td>已发送</td>
                 		</#if>
                 		<#if party.statusId?has_content && party.statusId=="ORDER_CREATED">
                 			<td>已创建</td>
                 		</#if>
                 		<#if party.statusId?has_content && party.statusId=="ORDER_CANCELLED">
                 			<td>已取消</td>
                 		</#if>
                 		--> 
                 		<td>${party.city!}${party.address1!}</td>
                 		<td>${party.postalCode!}</td>
                 		<#-- 
                 		<td><@ofbizCurrency amount=party.totalGrandAmount isoCode="USD" /></td>
                 		-->
                 		<#--${party.totalGrandAmount!}  --> 
