<div class="wrap">
    <div id="employee" style="min-height: 500px;">
        <form action="createPersonAndDept" name="MyForm" id="MyForm" method="post">
        <div class="employee_block">
            <h3>添加团队成员<em></em></h3>
            <ul class="employee_info">
                <li>
                    <label class="title">单位：</label>${entity.deptName}
                </li>
                <li>
                    <label class="title">人员：</label>
                    <input class="inp-text kingkong-input name selectTransferPut" id="customerName" type="text" name="customerName" value=""/>
                    <input type="hidden" name="customerId" id="customerId" value="">
                    <input type="hidden" name="deptId" id="deptId" value="${parameters.deptId!}">
	                 <#if customerList?has_content>
				        <ul class="transferAccountsSelect" id="transferAccountsSelect" style="display: none;max-height:500px;height: 119px;top: 82px;left: 119px;">
				          <#list customerList as item>
				            <li title="${(item.userLoginId)!}" partyId="${(item.partyId)!}" alt="${(item.userLoginId)!}" style="margin-top: 0px;">
