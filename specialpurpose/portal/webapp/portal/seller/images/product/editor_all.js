...
(function () {
    ...
    function setValue( form, editor ) {
        ...
        textarea.value = editor.hasContents() ?
            (editor.options.allHtmlEnabled ? editor.getAllHtml() : editor.getContent(null,null,false)):
            ''
    }
        ...
        setContent:function ( html, isAppendTo,notFireSelectionchange ) {
			...
			if ( serialize ) {
				...
				if( window.jQuery){
					 ...
					 fragment.innerHTML = '<p class="holder">1</p>'+html; 
					 var styles   = jQuery('style',fragment).attr('id','customstyle');
					 var head     = jQuery('head',this.document);
					 jQuery('#customstyle',head).remove(); 
					 head.append(styles);
					 fragment = null; 
				} 
            ...
            //trace:1559
			html = html.replace( new RegExp( '[\r' + domUtils.fillChar + ']*', 'g' ), '' );
            if(window.jQuery){
				$(this.body).html( html );	
			}else{
...
UE.ajax = function() {
    ...
    var $ajax = {
		...
		request:function(url, ajaxOptions) {
			...
			var $now = new Date() - 0;
            var str = url + (url.indexOf("?")==-1?"?":"&") + (method=="POST"?"":submitStr+ "&noCache=" + $now);
			ajaxRequest.open(method, str, ajaxOpts.async);
			ajaxRequest.onreadystatechange = function() {
				if (ajaxRequest.readyState == 4) {
					if (!timeIsOut && ajaxRequest.status == 200) {ajaxOpts.onsuccess(ajaxRequest);} else {ajaxOpts.onerror(ajaxRequest);}
				}
			};
			if (method == "POST") {ajaxRequest.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');ajaxRequest.send(submitStr);} else {
        ...
        switch ( node.tag ) {
            ...
            case 'img':
                ...
                if ( node.attributes.src && /^(?:file)/.test( node.attributes.src ) ) {
                    ...
                    node.attributes.style="background:url(" + (me.options.errorBg)+") no-repeat 5px 5px; border:1px solid ;border-color:#a0a0a0 #e3e3e3 #e3e3e3 #a0a0a0;";
            ...
            case 'font':
                ...
                var $object = {};
				if(attr.size){
					$object['font-size'] = (sizeMap[attr.size] || 12) + 'px'
				}
				if(attr.color){
					$object['color'] = attr.color
				}
				if(attr.face){
					$object['font-family'] = attr.face ;
				}
				node.attributes = {
                ...
                while(node.parent.tag == node.tag && node.parent.children.length == 1){
                    if(node.attributes.style && node.parent.$styles){
						$parent_object =  node.parent.$styles;
						$.extend($parent_object,$object);  
						$css = [];
						for(var i in $parent_object){
							if(Object.hasOwnProperty.call($parent_object,i)){
								$css.push(i+':'+$parent_object[i]);
							}
						} 
						node.parent.attributes.style = $css.join(';');
					}
    ...
    me.ready(function () {
        utils.cssRule('table',
            ...
            '.selectTdClass{background-color:#edf5fa !important}' +
                ...
                'table.border td,table.border th{border: 1px solid #DDD;} .ue-table-interlace-color-single{ background-color: #fcfcfc; } .ue-table-interlace-color-double{ background-color: #f7faff; }', me.document);
    ...
    UE.commands['inserttable'] = {
        ...
        execCommand:function (cmd, opt) {
            function createTable(opt, tableWidth, tdWidth) {
                ...
                return '<table width="' + tableWidth + '"   class="border"><tbody>' + html.join('') + '</tbody></table>'
            }
...
UE.plugins['contextmenu'] = function () {
    ...
    me.addListener( 'contextmenu', function ( type, evt ) {
		...
		var cmdName = 'tablealignment'; 
	    if (!((me.commands[cmdName] || UE.commands[cmdName]) &&( me.queryCommandState(cmdName)) > -1)) {
			return ;
		} 
...
(function () {
    ...
    var btnCmds = ['undo', 'redo', 'formatmatch',
        ...
        'deletecol', 'splittorows', 'splittocols', 'splittocells', 'mergecells', 'deletetable','upload','albums', 'relatedproduct'];
        ...
        editorui[ci] = function (cmd) {
            return function (editor) {
                var ui = new editorui.Button({
                    ...
                    showText:editor.options.labelMap[cmd],
					label   :editor.options.labelMap[cmd] 
                });
        ...
        initEditorUI:function () {
			...
			*/	
            ...
            function setCount(editor,ui) {
                ...
                var count = editor.getContentLength();
                if (count > max) {
