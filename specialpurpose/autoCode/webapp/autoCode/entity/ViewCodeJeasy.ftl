...
<div id="content-container" class="container ptz">
  <div class="es-row-wrap container-gap">
	  <div class="row">
	    <div class="col-md-12">
	      <div class="page-header"><h1>拷贝代码至项目下后,运行 ant load-demo ||ant start
	      	 <button type="button" onclick="javascript:document.ViewCode.submit();" class="btn btn-fat btn-primary btn-large">生成代码文件</button>
	      	 </h1>
	      </div>
	    </div>
	  </div>
...
<ul class="nav nav-tabs" id="myTab"  role="tablist"> 
	<li class="active"><a href="#tab1" role="tab" data-toggle='myTab'>controller.xml</a></li> 
    <li><a href="#tab2" data-toggle='myTab'role="tab">screens.xml</a></li> 
    <li><a href="#tab3" data-toggle='myTab'role="tab">forms.xml</a></li> 
    <#if requestAttributes.parameters.formsType=="ftl"&&!relationEntity??>
    	<li><a href="#tab4" data-toggle='myTab'role="tab">ftl编辑</a></li> 
    </#if>
    <#if relationEntity??>
    	<li><a href="#tab5" data-toggle='myTab'role="tab">主从编辑</a></li> 
    </#if>
    <#if requestAttributes.parameters.listType=="ftl">
    	<li><a href="#tab6" data-toggle='myTab'role="tab">列表ftl</a></li> 
    </#if>
    <#if requestAttributes.parameters.listType=="grid">
    	<li><a href="#tab7" data-toggle='myTab'role="tab">列表jqGrid</a></li> 
    	<li><a href="#tab8" data-toggle='myTab'role="tab">列表-groovy</a></li> 
	</#if>
    <li><a href="#tab9" data-toggle='myTab'role="tab">service.xml</a></li> 
    <#if requestAttributes.parameters.javaType.equals("simple")>
		<li><a href="#tab10" data-toggle='myTab'role="tab">simple.xml</a></li> 
	</#if>
	<#if requestAttributes.parameters.hasMultipart.equals("Y")>
		<li><a href="#tab11" data-toggle='myTab'role="tab">java-Events</a></li> 
	</#if>
	<#if requestAttributes.parameters.javaType.equals("java")>
		<li><a href="#tab12" data-toggle='myTab'role="tab">java</a></li> 
	</#if>
	<li><a href="#tab13" data-toggle='myTab'role="tab">种子数据</a></li> 
	
</ul> 
<div class="tab-content"> 
	<div class="tab-pane active" id="tab1">
<h2>${projectName}\webapp\${projectName}\WEB-INF\${modelNameLower}-controller.xml<button class="btn btn-fat btn-primary btn-large" onclick="copyCode('area1');return false;">复制</button></h2>	
<textarea cols='150' rows='28' name='controlText' id='area1' style="width:100%">
	<!--${entityTitle!}-->
	...
	<request-map uri="create${entityName}">
	    ...
	    <response name="success" type="request-redirect"  value="Find${entityName}"/>
		<#else>
	...
	<#if requestAttributes.parameters.listType=="grid">
	<request-map uri="get${entityName}">
	    <security https="false" auth="false"/>
	    <event type="groovy" path="component://${projectName}/webapp/${projectName}/WEB-INF/actions/${modelNameLower}/get${entityName}.groovy" />
	...
	<!--${entityTitle!}-view -->
	<view-map name="Find${entityName}" type="screen" 
		page="component://${projectName}/widget/${modelName}Screens.xml#Find${entityName}" />
	<view-map name="Edit${entityName}" type="screen" 
	...
	<#if hasSearchAdvanced.equals("Y")>
	<view-map name="Find${entityName}Advanced" type="screen" page="component://${projectName}/widget/${modelName}Screens.xml#Find${entityName}Advanced" />
	</#if>
	<#if relationEntity??>
	<view-map name="Edit${entityName!}And${relationEntity!}" type="screen" 
		page="component://${projectName!}/widget/${modelName!}Screens.xml#Edit${entityName!}And${relationEntity!}" />
	</#if>
</textarea>
</div>
<div class="tab-pane" id="tab2">
 <h2>${projectName}\widget\${modelName}Screens.xml<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('area2');return false;">复制</button></h2>	
<textarea cols='150' rows='28' name='screensText' id='area2' style="width:100%">
	<!--${entityTitle!} -->
	<screen name="Find${entityName}">
		<section>
			<actions>
				...
				<#if requestAttributes.parameters.listType=="grid">
				<set field="operationButton[]" value="编辑|icon-edit|edit${entityName}Grid()|jsFunction|${entityName}_UPDATE" />	
				<#else>
				<set field="operationButton[]" value="编辑|icon-edit|Edit${entityName}?${entityId}=|editSelected|${entityName}_UPDATE" />
				</#if>
				
				<#if hasDeleteAll.equals("Y")> 
				<#if requestAttributes.parameters.listType=="grid">
				<set field="operationButton[]" value="删除|icon-remove|deleteAll${entityName}Grid()|jsFunction|${entityName}_DELETE" />
				<#else>
			...
			<widgets>
				<decorator-screen name="main-decorator" location="${r"${parameters.mainDecoratorLocation}"}">
					<decorator-section name="body">
						...
						<#else>
							<#--<decorator-screen name="ListResults" location="${r"${parameters.mainDecoratorLocation}"}">
								<decorator-section name="list-results">
								</decorator-section>
							</decorator-screen>
							-->
							<platform-specific>
					          	<html><html-template location="component://${projectName}/webapp/${projectName}/${modelNameLower}/${entityName}_list.ftl"/></html>
					        </platform-specific>
						</#if>
					</decorator-section>
				</decorator-screen>
		...
		<section>
			...
			<widgets>
				<decorator-screen name="main-decorator" location="${r"${parameters.mainDecoratorLocation}"}">
					...
					<#else>
						<decorator-screen name="<#if requestAttributes.parameters.hasWorkflow.equals("Y")>DetailWorkflowScreenDecorator<#else><#if requestAttributes.parameters.hasMultipart.equals("Y")>DetailUploadScreenDecorator<#else>DetailScreenDecorator</#if></#if>" location="${r"${parameters.mainDecoratorLocation}"}">
...
<h2>forms.xml [${projectName}\widget\${modelName}Forms.xml]<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('area3');return false;">复制</button></h2>	
	<textarea cols='150' rows='28' name='formsText' id='area3' style="width:100%">
	<!--${entityTitle!} -->
<#if hasSearchAdvanced.equals("Y")> 
...
<#if requestAttributes.parameters.listType=="forms">
	<form name="Find${entityName}" type="single"  style="form-vertical"  target="Find${entityName}" default-entity-name="${entityName}">
	...
	<form name="List${entityName}" type="list" list-name="listIt" group-columns="false"   paginate-target="Find${entityName}"
   		default-entity-name="${entityName}" odd-row-style="alternate-row" header-row-style="header-row-2" default-table-style="table table-bordered table-striped table-condensed">
		<actions>
	...
	<form name="Edit${entityName}"  <#if requestAttributes.parameters.hasMultipart.equals("Y")>type="upload"<#else>type='single'</#if> style="form-horizontal" 
		...
		<field name="${field!}" position='${fieldIndex}'  use-when="${'$'}{groovy:oper==null}" widget-style=" ${required!}" title="${title!}">
			<drop-down text-size="30" allow-empty="true" no-current-selected-key="Y">
		...
		<field name="save" title="保存"  widget-style="btn btn-primary"><submit/></field>
	</form>
</#if>
<#if hasLookup.equals("Y")>
	<form name="lookup${entityName}" target="Lookup${entityName}" title="" type="single"
	    header-row-style="header-row" default-table-style="basic-table">
	    <field name="${entityId}" ><text-find  hide-options="true"/></field>
	    <#list searchField as field>
	    <field name="${field}" ><text-find  hide-options="true"/></field>
	    </#list>
...
<div class="tab-pane" id="tab4">
  <h2>
	...
	<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('editFtlText');return false;">复制编辑FTL</button>
	</h2>	
	<textarea cols='150' rows='28' id='editFtlText' name='editFtlText' style="width:100%">
		<#include "component://autoCode/webapp/autoCode/entity/ViewCodeJeasyEditFtl.ftl"/>
	</textarea>
	</div>
</#if>
...
<!--主从结构FTL-->
	<div class="tab-pane" id="tab5">
	 <h2>
		...
		<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('editAndFtlText');return false;">复制主从FTL</button>
		</h2>	
		<textarea cols='150' rows='28' id='editAndFtlText' name='editAndFtlText' style="width:100%">
			<#include "component://autoCode/webapp/autoCode/entity/ViewCodeJeasyEditFtl.ftl"/>
					<br>
					<legend>从表信息  <button class="btn btn-fat btn-primary btn-large"class="btn" onclick="javascript:commonAddRows('studentTable');return false;">新增</button></legend>
					<input type='hidden'  value='1' id='rowNum'>
		             
		            <table  id='${relationEntity!}Table'  class="table table-bordered table-striped table-condensed">
		    			<tbody>
					    <tr class="header-row">
					    <td width='2%'>选择</td>
					    <td width='2%'>序号</td>
				 ...
				 <div class="form-actions">
							<button class="btn btn-fat btn-primary btn-large"type="submit" class="btn btn-primary">保存</button>
							<button class="btn btn-fat btn-primary btn-large"class="btn">重置</button>
				  </div>
			</div>
		</div>
...
<div class="tab-pane" id="tab6">
 <h2>
	...
	<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('listFtlText');return false;">复制列表FTL</button>
	</h2>	
	<textarea cols='150' rows='28' id='listFtlText' name='listFtlText' style="width:100%">
...
<div class="tab-pane" id="tab7">
	<h2>列表grid <button class="btn btn-fat btn-primary btn-large"onclick="copyCode('easyuiFTLText');return false;">复制列表jqGrid</button>
	${projectName}/webapp/${projectName}/${modelNameLower}/${entityName}_list.ftl
	</h2>	
	<textarea cols='150' rows='28' id='easyuiFTLText' name='easyuiFTLText' style="width:100%">
	<script language='javascript'>
	$(function(){
		$('#${entityName}_gridTb').jqGrid({
			url:'get${entityName}',
			datatype: "json",
		   	multiselect: true,
			multiboxonly: true,
			<#assign colNamesStr = '[\''+entityId+'\''/>
			<#assign colModelStr = '[{name:\''+entityId+'\',checkbox:true,hidden:true,width:55}'/>
			<#if listField?size&gt;0>
				<#assign colNamesStr = colNamesStr+','/>
				<#assign colModelStr = colModelStr+','/>
			</#if>
			<#list listField as field>
				<#assign listFieldType=listFieldMap["listFieldType_"+field]?if_exists>
				<#assign listFieldHasSort=listFieldMap["listFieldHasSort_"+field]?if_exists>
	        	...
	        	<#--构造colNames串-->
	        	<#assign colname = title!/>
	        	<#if listFieldType=='select'>
	        	   	<#assign colname = colname+'\',\''+displayName!/>
	        	</#if>
	        	<#assign colNamesStr = colNamesStr+'\''+colname+'\''>
	        	<#if field_index!=listField?size-1>
	        		<#assign colNamesStr = colNamesStr+','/>
	        	</#if>
	        	<#--构造colModel串-->
	        	<#assign colModelLine = '{name:\''+field+'\',width:100,editable:true'/>
	        	<#if listFieldHasSort=='true'>
	        		<#assign colModelLine = colModelLine+',index:\''+field+'\''/>
	    		</#if>
	        	<#if field==entityId>
	        		<#assign colModelLine = colModelLine+',checkbox:true}'/>
	        	<#elseif listFieldType=='select'>
	        		<#assign colModelLine = colModelLine+',hidden:true},'/>
	        		<#assign colModelLine = colModelLine+'{name:\''+listFieldRelName+'Fk\',width:100,editable:true,index:\''+listFieldRelName+'Fk\'}'/>
	    		<#else>
	    			<#assign colModelLine = colModelLine+'}'/>
	        	</#if>
	        	<#assign colModelStr = colModelStr+colModelLine/>
	        	<#if field_index!=listField?size-1>
	        		<#assign colModelStr = colModelStr+','/>
	        	</#if>
	        </#list>
			...
			<#assign colModelStr = colModelStr+']'/>
		   	...
		   	height: 400,
		   	rowList:[10,20,30],
		   	pager: '#${entityName}_pager',
		    viewrecords: true,
		    altRows: true,
		    editurl:'OperGenericValue?entityName=${entityName}&idField=${entityId}',
		    jsonReader : {  
	          repeatitems : false,
			  id: "0"
	        }
		});
		//jQuery("#${entityName}_gridTb").jqGrid('navGrid','#${entityName}_pager',{edit:true,add:true,del:true});
	});		
	
	function ${entityName}_gridReload(){
		var targetUrl = "get${entityName}?entityName=${entityName}";
		var form = $('#${entityName}_gridTb_search');
		$.each(form.serializeArray(), function(index) {
			var key = this['name'];
			var val = this['value'];
			targetUrl += ("&"+key+"="+val);
		});
		jQuery("#${entityName}_gridTb").jqGrid('setGridParam',{url:targetUrl,page:1}).trigger("reloadGrid");
	}
	function ${entityName}_create(){
		window.location.href="Edit${entityName}";
	}
	function edit${entityName}Grid(){
		var rowids = $( "#${entityName}_gridTb" ).getGridParam( "selarrrow" ); 
		if(rowids.length==1){
			window.location.href="Edit${entityName}?${entityId}="+rowids;
		}else {
			alertDialog('只能选择一条要修改的记录！');
		}
	}
	function deleteAll${entityName}Grid(){
		var rowids = $( "#${entityName}_gridTb" ).getGridParam( "selarrrow" ); 
		if(rowids.length>0){
			window.location.href="deleteAll${entityName}?ids="+rowids;
		}else {
	...
	<div class="row-fluid sortable">	
		<div class="box span12">
			<div class="box-content" >
				<form id='${entityName}_gridTb_search' class='form-horizontal'>
					<#list searchField as field>
						<#assign editFieldType=editFieldMap["editFieldType_"+field]>
						<#assign editFieldRelName=''>
						<#assign required=''>
					...
					<input type="button" onclick="javascript:${entityName}_gridReload();" href="javascript:void(0);" class="btn btn-primary" name="${entityName}_search" value="查询">
					<input type="button" onclick="javascript:${entityName}_create();return false;" class="btn btn-primary" value="新建">           
					<input type="button" onclick="javascript:edit${entityName}Grid();return false;" class="btn btn-primary" value="修改">           
					<input type="button" onclick="javascript:deleteAll${entityName}Grid();return false;" class="btn btn-primary" value="删除">     
				</form>
	
				<div id="dataGrid">
				<table id="${entityName}_gridTb"></table>
				<div id="${entityName}_pager"></div>
				</div>
...
<h2>
${projectName}/webapp/${projectName}/WEB-INF/actions/${modelNameLower}/get${entityName}.groovy
<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('easyuiGroovyText');return false;">复制列表easyui-groovy</button>
</h2>	
<textarea cols='150' rows='28' id='easyuiGroovyText' name='easyuiGroovyText' style="width:100%">
	...
	request.setAttribute("total",BigDecimal.valueOf((listSize+viewSize-1)).divide(BigDecimal.valueOf(viewSize)).intValue());
...
<#if requestAttributes.parameters.listType=="easyui">
<div title="列表easyuiFTL" style="padding:10px">	
<h2> <button class="btn btn-fat btn-primary btn-large"onclick="copyCode('easyuiFTLText');return false;">复制</button>
${projectName}/webapp/${projectName}/${modelNameLower}/${entityName}_list.ftl
</h2>	
<textarea cols='150' rows='28' id='easyuiFTLText' name='easyuiFTLText' style="width:100%">
<script language='javascript'>
$(function(){
	$('#${entityName}_gridTb').datagrid({
		url:'get${entityName}?thruDate_op=empty',
		fit:true,
		pagination:true,
...
<div class="tab-pane" id="tab9">
 <h2>${projectName}\servicedef\services_${modelNameLower}.xml<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('area4');return false;">复制</button></h2>	
<textarea cols='150' rows='28' name='serviceText' id='area4' style="width:100%">
	<!--${entityTitle!} -->
	<#if requestAttributes.parameters.javaType.equals("simple")>
	<service name="create${entityName}" engine="simple" default-entity-name="${entityName}"
             location="component://${projectName}/script/${modelName}Services.xml" invoke="create${entityName}" auth="false">
     ...
     <service name="update${entityName}" engine="simple" default-entity-name="${entityName}"
             location="component://${projectName}/script/${modelName}Services.xml" invoke="update${entityName}" auth="false">
    ...
    <service name="delete${entityName}" engine="simple" default-entity-name="${entityName}"
             location="component://${projectName}/script/${modelName}Services.xml" invoke="delete${entityName}" auth="false">
    ...
    <service name="deleteAll${entityName}" engine="simple" default-entity-name="${entityName}"
             location="component://${projectName}/script/${modelName}Services.xml" invoke="deleteAll${entityName}" auth="false">
        <description>Delete ${entityName}-${entityTitle!}</description>
    ...
    <service name="create${entityName}" engine="java" default-entity-name="${entityName}"
     		location="${packageName}.${projectName}.${modelNameLower}.${entityName}Services" invoke="create${entityName}" auth="false">
        <description>Create ${entityName}-${entityTitle!}</description>
        <auto-attributes include="pk" mode="INOUT" optional="true" />
		<auto-attributes include="nonpk" mode="IN" optional="true" />
    </service>
     <service name="update${entityName}" engine="java" default-entity-name="${entityName}"
             location="${packageName}.${projectName}.${modelNameLower}.${entityName}Services" invoke="update${entityName}" auth="false">
        <description>Update ${entityName}-${entityTitle!}</description>
        <auto-attributes include="pk" mode="IN" optional="false" />
		<auto-attributes include="nonpk" mode="IN" optional="true" />
    </service>
    <service name="delete${entityName}" engine="java" default-entity-name="${entityName}"
             location="${packageName}.${projectName}.${modelNameLower}.${entityName}Services" invoke="delete${entityName}" auth="false">
        <description>Delete ${entityName}-${entityTitle!}</description>
        <auto-attributes mode="IN" include="pk" optional="false"/>
    </service>
    <service name="deleteAll${entityName}" engine="java" default-entity-name="${entityName}"
              location="${packageName}.${projectName}.${modelNameLower}.${entityName}Services" invoke="deleteAll${entityName}" auth="false">
        <description>Delete ${entityName}-${entityTitle!}</description>
        <attribute name="ids"  mode="IN" type="String" optional="false"/>
    </service>
    <#if relationEntity??>
    <service name="createOrUpdate${relationEntity}" engine="simple" default-entity-name="LimsStudent"
             location="component://insure/script/MedicalServices.xml" invoke="createOrUpdate${relationEntity}" auth="false">
...
<#if requestAttributes.parameters.hasWorkflow=="Y">
	...
	<service name="saveExamin${entityName}" engine="simple" default-entity-name="${entityName}"
             location="component://${projectName}/script/${modelName}Services.xml" invoke="saveExamin${entityName}" auth="false">
...
<h2>${projectName}\script\${modelName}Services.xml<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('area5');return false;">复制</button></h2>	
	<textarea cols='150' rows='28' name='simpleText' id='area5' style="width:100%">
    <!--${entityTitle!} -->
...
<#if requestAttributes.parameters.hasMultipart.equals("Y")>
	...
	<h2>${packageName}.${projectName}.${modelNameLower}.${entityName}Events.java
	<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('EventsJavaText');return false;">复制</button>
	</h2>	
	<textarea cols='150' rows='28' id='EventsJavaText' name='EventsJavaText' style="width:100%">
...
<#if requestAttributes.parameters.javaType.equals("java")>
	...
	<button class="btn btn-fat btn-primary btn-large"onclick="copyCode('servicesJavaText');return false;">复制</button>
	</h2>	
	<textarea cols='150' rows='28' id='servicesJavaText' name='servicesJavaText' style="width:100%">
...
public class ${entityName}Services 
{
     ...
     *         parameters.
     */
    public static Map<String, Object> deleteAll${entityName}(DispatchContext dctx, Map<String, ? extends Object> context)
    {
        Map<String, Object> result = FastMap.newInstance();
        Delegator delegator = dctx.getDelegator();
        ...
        try
        {
    		...
    		for(int i=0;i<idList.size();i++){
    			dispatcher.runSync("delete${entityName}", UtilMisc.toMap("${entityId}",idList.get(i),"userLogin",userLogin));
    		}
         } catch (Exception e)
        {
            Debug.logError(e, module);
...
<h2>权限(SecurityData.xml)和菜单初始数据(MenuTreeData.xml) <button class="btn btn-fat btn-primary btn-large"onclick="copyCode('area6');return false;">复制</button></h2>	
<textarea cols='150' rows='28' name='dataText' id='area6' style="width:100%">
	...
	<MenuTree id="${entityName}Mgr" itemName="${entityTitle!}管理"  parentItemId="YxckModel" rank="1"/>
