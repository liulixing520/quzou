 ...
 */
public class MultiShippingEvents {

	public static final String module = MultiShippingEvents.class.getName();
	...
	public static Map<String, Object> getShipGroupEstimate(LocalDispatcher dispatcher, Delegator delegator, String orderTypeId, String shipmentMethodTypeId, String carrierPartyId, String carrierRoleTypeId, String shippingContactMechId, String productStoreId, List<Map<String, Object>> itemInfo, BigDecimal shippableWeight, BigDecimal shippableQuantity, BigDecimal shippableTotal, String partyId, String productStoreShipMethId) {
		return getShipGroupEstimate(dispatcher, delegator, orderTypeId, shipmentMethodTypeId, carrierPartyId, carrierRoleTypeId, shippingContactMechId, productStoreId, null, itemInfo, shippableWeight, shippableQuantity, shippableTotal, partyId, productStoreShipMethId);
	}
	...
	public static Map<String, Object> getShipGroupEstimate(LocalDispatcher dispatcher, Delegator delegator, String orderTypeId, String shipmentMethodTypeId, String carrierPartyId, String carrierRoleTypeId, String shippingContactMechId, String productStoreId, String supplierPartyId, List<Map<String, Object>> itemInfo, BigDecimal shippableWeight, BigDecimal shippableQuantity, BigDecimal shippableTotal, String partyId, String productStoreShipMethId) {
		...
		// ShipmentCostEstimate entity allows null value for geoIdTo field. So
		// if geoIdTo is null we should be using orderFlatPrice for shipping
		// cost.
		// So now calcShipmentCostEstimate service requires
		// shippingContactMechId only if geoIdTo field has not null value.
		// if (shippingContactMechId == null) {
		// errorMessageList.add("Please Select Your Shipping Address.");
		// return ServiceUtil.returnError(errorMessageList);
		// }

		// if as supplier is associated, then we have a drop shipment and should
		// use the origin shipment address of it
	...
	public static BigDecimal getExternalShipEstimate(LocalDispatcher dispatcher, GenericValue storeShipMeth, Map<String, Object> context) throws GeneralException {
		...
		if (serviceName != null) {
			...
			if (UtilValidate.isNotEmpty(serviceName)) {
				...
				// @add by WuHK
				context.put("productStoreShipMethId", storeShipMeth.getString("productStoreShipMethId"));
				// invoke the service
				...
				try {
					...
					// because we don't want to blow up too big or rollback the
					// transaction when this happens, always have it run in its
					// own transaction...
				...
				if (ServiceUtil.isError(serviceResp)) {
					...
					// getShipGroupEstimate would return an error, causing all
					// sorts of services like add or update order item to abort
				} else {
	...
	/**
	 * Attempts to get the supplier's shipping origin address and failing that,
	...
	public static List<GenericValue> filterShippingMethods(Delegator delegator, List<GenericValue> carrierShipmentMethodList, String shippingContactMechId) {
		...
		try {
			for (GenericValue carrierShipmentMethod : carrierShipmentMethodList) {
				String productStoreShipMethId = carrierShipmentMethod.getString("productStoreShipMethId");
				...
				if (address != null) {
					...
					if (productStoreShipConfig != null)
						returnList.add(carrierShipmentMethod);
				}
			}
		} catch (Exception e) {
			Debug.logError(e, module);
