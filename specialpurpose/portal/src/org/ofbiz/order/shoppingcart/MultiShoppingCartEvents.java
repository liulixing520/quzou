 ...
 */
public class MultiShoppingCartEvents {

    public static String module = MultiShoppingCartEvents.class.getName();
    ...
    public static String addToCart(HttpServletRequest request, HttpServletResponse response) {
		...
		ShoppingCart cart = getCartObject(request, null, null, productStoreId);
		...
		if (productStore != null) {
			...
			if ("Y".equals(addToCartReplaceUpsell)) {
			    ...
			    EntityCondition cond = EntityCondition.makeCondition(
				    UtilMisc.toList(EntityCondition.makeCondition("productIdTo", EntityOperator.EQUALS, productId), EntityCondition.makeCondition("productAssocTypeId", EntityOperator.EQUALS, "PRODUCT_UPGRADE")), EntityOperator.AND);
		...
		result = cartHelper.addToCart(catalogId, shoppingListId, shoppingListItemSeqId, productId, productCategoryId, itemType, itemDescription, price, amount, quantity, reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId,
    ...
    public static String addProductPromoCode(HttpServletRequest request, HttpServletResponse response) {
    	...
    	if (UtilValidate.isNotEmpty(promoCodeId)) {
    ...
    /**
     * Adds all products in a category according to default quantity on
     * ProductCategoryMember for each; if no default for a certain product in
     * the category, or if quantity is 0, do not add
     */
	...
	MultiShoppingCartMap shoppingCartMap = getCartMap(request);
	List<ShoppingCart> shoppingCartList = shoppingCartMap.getShoppingCartList();
	for (ShoppingCart cart : shoppingCartList) {
	    String productStoreId = cart.getProductStoreId();

	    String prefix = "PS" + productStoreId + "_";
	    ShoppingCartHelper cartHelper = new ShoppingCartHelper(null, dispatcher, cart);
	    String controlDirective;
	    Map<String, Object> result;
	    // not used yet: Locale locale = UtilHttp.getLocale(request);

	    Map<String, Object> tempParamMap = UtilHttp.getParameterMap(request);
	    Map<String, Object> paramMap = UtilHttp.makeParamMapWithPrefix(tempParamMap, null, prefix, null);
	    paramMap.putAll(UtilHttpExt.makeParamMapWithoutPrefix(tempParamMap, null, prefix, null));
	...
	MultiShoppingCartMap cartMap = getCartMap(request);
	cartMap.clear();
    ...
    public static ShoppingCart getCartObject(HttpServletRequest request, Locale locale, String currencyUom, String productStoreId) {
	LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
	...
	/*
	 * if(UtilValidate.isEmpty(productStoreId)){ productStoreId =
	 * ProductStoreWorkerExt.getProductStoreId(request,productStoreId); }
	 */
	...
	if (UtilValidate.isNotEmpty(productStoreId)) {
	    if (cart == null || !productStoreId.equals(cart.getProductStoreId())) {
		if (cartMap.containsKey(productStoreId)) {
		    cart = cartMap.getFromCartMap(productStoreId);
		} else {
		    cart = new MultiWebShoppingCart(request, locale, currencyUom, productStoreId);
		}
		session.setAttribute("shoppingCart", cart);
	    }
	}
	...
	if (UtilValidate.isNotEmpty(cart)) {
	    cartMap.addToCartMap(cart.getProductStoreId(), cart);
	    session.setAttribute("shoppingCartMap", cartMap);
	}
	...
	return getCartObject(request, null, null, null);
    }

    public static MultiShoppingCartMap getCartMap(HttpServletRequest request) {
	HttpSession session = request.getSession(true);
	// shoppingCartMap
	MultiShoppingCartMap cartMap = (MultiShoppingCartMap) session.getAttribute("shoppingCartMap");
	if (UtilValidate.isEmpty(cartMap)) {
	    cartMap = new MultiShoppingCartMap();
	    session.setAttribute("shoppingCartMap", cartMap);
	}
	return cartMap;
    }

    public static List<ShoppingCart> getCartList(HttpServletRequest request) {
	HttpSession session = request.getSession(true);
	...
	for (ShoppingCart cart : shoppingCartList) {
	    ...
	    if (cart.getCurrency() == null || !currencyUom.equals(cart.getCurrency())) {
		try {
		    cart.setCurrency(dispatcher, currencyUom);
		} catch (CartItemModifyException e) {
	...
	singleKeepCartUpdated(request, response);
	return "success";
    }

    /** Update the cart's UserLogin object if it isn't already set. */
    public static String singleKeepCartUpdated(HttpServletRequest request, HttpServletResponse response) {
	LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
	HttpSession session = request.getSession();
	ShoppingCart cart = getCartObject(request);
    ...
    /**
     * For GWP Promotions with multiple alternatives, selects an alternative to
     * the current GWP
     */
	...
	if (cartLine.getIsPromo()) {
		...
		if (UtilValidate.isNotEmpty(checkOrderAdjustment.getString("productPromoId")) && UtilValidate.isNotEmpty(checkOrderAdjustment.getString("productPromoRuleId"))
    ...
    /**
     * set the order name of the cart based on request. right now will always
     * return "success"
     *
     */
    ...
    /**
     * set the PO number of the cart based on request. right now will always
     * return "success"
     *
     */
	...
	ShoppingCart cart = null;
	try {
	    Map<String, Object> outMap = dispatcher.runSync("loadCartFromOrder", UtilMisc.<String, Object> toMap("orderId", orderId, "skipProductChecks", Boolean.TRUE, // the
																					// products
																					// have
																					// already
																					// been
	...
	// if the order mode is not set in the attributes, then order entry has
	...
	if (rowCount < 1) {
		...
		if ((quantityStr == null) || (quantityStr.equals(""))) { // otherwise,
		    ...
		    quantityStr = "0"; // default quantity is 0, so without a
				       // quantity input, this field will not be
				       // added
		}
