...
if (orderHeader) {
	...
	context.orderHStatusIdList = EntityUtil.getFieldListFromEntityList(orderHeaderStatuses, "statusId", true)
	...
	// get shipmentInfo
	shipmentInfo = dispatcher.runSync("getOrderShipmentInfo", [orderId : orderId]);
	context.carrierName = shipmentInfo.carrierName;
	context.carrierSiteName = shipmentInfo.carrierSiteName;
	context.carrierSymbol = shipmentInfo.carrierSymbol;
	context.trackingNumber = shipmentInfo.trackingNumber;
...
// This case comes when order's shipping amount is  more then or less than default percentage (defined in shipment.properties) of online UPS shipping amount.
    ...
    //下单人
    placingPartyRole = EntityUtil.getFirst(delegator.findByAnd("OrderRole", [orderId : orderId, roleTypeId : "PLACING_CUSTOMER"]));
    context.placingPartyRole=placingPartyRole;
    
    //送货状态
    shipmentStateHistorys=org.ofbiz.ebiz.order.OrderServices.getOrderShipmentStateHistory(delegator,orderId);
    context.shipmentStateHistorys=shipmentStateHistorys;
    
    //退货状态
    returnId = org.ofbiz.ebiz.order.OrderServices.getReturnOrderByOrderId(delegator,orderId);
	if(returnId!=null&&returnId.length()>0){
		...
		EntityConditionList<EntityExpr> whereConditions = EntityCondition.makeCondition(UtilMisc.toList(
	            ...
	            EntityCondition.makeCondition("returnItemSeqId", EntityOperator.EQUALS, null)), EntityOperator.AND);
		returnStatuses=delegator.findList("ReturnStatus", whereConditions, null, null, null, false);
		if(returnStatuses!=null){
			context.returnStatuses=returnStatuses;
		}
	}
