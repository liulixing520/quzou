...
function subSearch(formName){
	var  searchType=document.getElementById('searchCType').value;
	if(searchType!=null&&searchType.length>0){
		if(searchType=='orderId'){
			document.getElementById('orderIdC').value=document.getElementById('searchCContent').value;
		}else if(searchType=='userLoginId'){
			document.getElementById('userLoginIdC').value=document.getElementById('searchCContent').value;
		}
...
<div id='pageContent'  class="pageContent" >
	<div class="panelBar">
		<ul class="toolBar">
		<form action='FindCreatedOrder' id='FindCreatedOrder' name='FindCreatedOrder' method="post" rel="pagerForm">
			  ...
			  <input type='hidden' name='orderId'  id='orderIdC' />
			  <input type='hidden' name='userLoginId' id='userLoginIdC' value='${requestParameters.userLoginId?if_exists}'/>
			  ...
			  <input type='hidden' name='orderStatusId' value='ORDER_CREATED'/>
			...
			<li><select name='searchType' id='searchCType'><option value='orderId'>订单号</option><option value='userLoginId'>下单人</option></select></li>
			<li><input type='text' name='searchContent' id='searchCContent' onblur="if(this.value=='') this.value=''" onfocus="if(this.value=='') this.value=''" value="" ></li>
			<li><a class="search" href="#"  onclick="javascript:subSearch('FindCreatedOrder');" ><span>搜索</span></a></li>
			<li><a class="search" href="<@ofbizUrl>FindCreatedOrderAdvanced</@ofbizUrl>" target="dialog" mask="true" title="查询框"><span>高级检索</span></a></li>
		</form>	
			</ul>
	</div>
    ...
    <!--dwz  分页 -->      
	<@dwzPagin orderList![] orderListSize!0 VIEW_SIZE_1!20 VIEW_INDEX_1!0  "FindCreatedOrder" "navTab"/>
</div>