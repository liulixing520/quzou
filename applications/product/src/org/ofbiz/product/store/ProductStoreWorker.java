    ...
    public static List<GenericValue> getAvailableStoreShippingMethods(Delegator delegator, String productStoreId, GenericValue shippingAddress, List<BigDecimal> itemSizes, Map<String, BigDecimal> featureIdMap, BigDecimal weight, BigDecimal orderTotal) {
        ...
        if (shippingMethods != null) {
            for (GenericValue method: shippingMethods) {
                ...
                //@WuHK add filter by cost config
                if(shippingAddress!=null){//PostalAddress
                	String productStoreShipMethId = method.getString("productStoreShipMethId");
					...
					GenericValue productStoreShipConfig = null;;
					try {
						productStoreShipConfig = delegator.findOne("ProductStoreShipConfig", false, UtilMisc.toMap("productStoreShipMethId", productStoreShipMethId, "shippingCountry", countryGeoId, "shippingProvince", stateProvinceGeoId));
					} catch (GenericEntityException e) {
						e.printStackTrace();
						continue;
					}
