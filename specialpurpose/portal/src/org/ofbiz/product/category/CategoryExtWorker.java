 ...
 */
public class CategoryExtWorker {

    public static final String module = CategoryExtWorker.class.getName();

    private CategoryExtWorker () {}
    ...
    public static void getRelatedCategories(ServletRequest request, String attributeName, boolean limitView) {
        ...
        productStoreId = UtilFormatOut.checkNull((String)requestParameters.get("productStoreId"), (String)request.getAttribute("optProductStoreId"), (String)request.getAttribute("productStoreId"));
        ...
        getRelatedCategories(request, attributeName,productStoreId, requestId, limitView);
    }

    public static void getRelatedCategories(ServletRequest request, String attributeName,String productStoreId, String parentId, boolean limitView) {
        getRelatedCategories(request, attributeName,productStoreId, parentId, limitView, false);
    }

    public static void getRelatedCategories(ServletRequest request, String attributeName,String productStoreId, String parentId, boolean limitView, boolean excludeEmpty) {
        List<GenericValue> categories = getRelatedCategoriesRet(request, attributeName,productStoreId, parentId, limitView, excludeEmpty);

        if (!categories.isEmpty())  request.setAttribute(attributeName, categories);
    }

    public static List<GenericValue> getRelatedCategoriesRet(ServletRequest request, String attributeName,String productStoreId, String parentId, boolean limitView) {
        return getRelatedCategoriesRet(request, attributeName,productStoreId, parentId, limitView, false);
    }

    public static List<GenericValue> getRelatedCategoriesRet(ServletRequest request, String attributeName,String productStoreId, String parentId, boolean limitView, boolean excludeEmpty) {
        return getRelatedCategoriesRet(request, attributeName,productStoreId, parentId, limitView, excludeEmpty, false);
    }

    public static List<GenericValue> getRelatedCategoriesRet(ServletRequest request, String attributeName, String productStoreId, String parentId, boolean limitView, boolean excludeEmpty, boolean recursive) {
      Delegator delegator = (Delegator) request.getAttribute("delegator");

      return getRelatedCategoriesRet(delegator, attributeName,productStoreId, parentId, limitView, excludeEmpty, false);
    }

    public static List<GenericValue> getRelatedCategoriesRet(Delegator delegator, String attributeName, String productStoreId, String parentProductCategoryId, boolean limitView, boolean excludeEmpty, boolean recursive) {
        List<GenericValue> categories = FastList.newInstance();

        if (Debug.verboseOn()) Debug.logVerbose("[CategoryWorker.getRelatedCategories] ParentID: " + parentProductCategoryId, module);

        List<GenericValue> rollups = null;

        try {
            rollups = delegator.findByAnd("ProductStoreCategoryRollup",
                        UtilMisc.toMap("productStoreId",productStoreId,"parentProductCategoryId", parentProductCategoryId),
                ...
                if (cv != null) {
                    if (excludeEmpty) {
                        if (!isCategoryEmpty(cv)) {
                            ...
                            categories.add(cv);
                            if (recursive) {
                                categories.addAll(getRelatedCategoriesRet(delegator, attributeName,productStoreId, cv.getString("productCategoryId"), limitView, excludeEmpty, recursive));
                            }
                        }
                    } else {
                        categories.add(cv);
    ...
    public static void getCategoryContentWrappers(Map<String, CategoryContentWrapper> catContentWrappers, List<GenericValue> categoryList, HttpServletRequest request) throws GenericEntityException {
        ...
        productStoreId = UtilFormatOut.checkNull((String)request.getParameter("productStoreId"), (String)request.getAttribute("optProductStoreId"), (String)request.getAttribute("productStoreId"));
        ...
        for (GenericValue cat: categoryList) {
            ...
            List<GenericValue> subCat = getRelatedCategoriesRet(request, "subCatList",productStoreId, productCategoryId, true);
            if (subCat != null) {
