...
function subSearch(formName){
	var  searchType=document.getElementById('searchAType').value;
	if(searchType!=null&&searchType.length>0){
		if(searchType=='orderId'){
			document.getElementById('orderIdA').value=document.getElementById('searchAContent').value;
		}else if(searchType=='userLoginId'){
			document.getElementById('userLoginIdA').value=document.getElementById('searchContentA').value;
		}
...
<div id='pageContent'  class="pageContent" >
	<div class="panelBar">
		<ul class="toolBar">
		<form action='FindApprovedOrder' id='FindApprovedOrder' name='FindApprovedOrder' method="post" rel="pagerForm">
			  ...
			  <input type='hidden' name='orderId'  id='orderIdA' />
			  <input type='hidden' name='userLoginId' id='userLoginIdA' value='${requestParameters.userLoginId?if_exists}'/>
			  ...
			  <input type='hidden' name='orderStatusId' value='ORDER_APPROVED'/>
			...
			<li><select name='searchType' id='searchAType'><option value='orderId'>订单号</option><option value='userLoginId'>下单人</option></select></li>
			<li><input type='text' name='searchContent' id='searchAContent' onblur="if(this.value=='') this.value=''" onfocus="if(this.value=='') this.value=''" value="" ></li>
			<li><a class="search" href="#"  onclick="javascript:subSearch('FindApprovedOrder');" ><span>搜索</span></a></li>
			<li><a class="search" href="<@ofbizUrl>FindApprovedOrderAdvanced</@ofbizUrl>" target="dialog" mask="true" title="查询框"><span>高级检索</span></a></li>
		</form>	
			</ul>
	</div>
    ...
    <!--dwz  分页 -->      
	<@dwzPagin orderList![] orderListSize!0 VIEW_SIZE_1!20 VIEW_INDEX_1!0  "FindApprovedOrder" "navTab"/>
</div>