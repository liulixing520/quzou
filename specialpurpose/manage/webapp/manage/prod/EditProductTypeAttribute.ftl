...
function checktype2(){
	...
	if('TEXT'== inputType){
		$("#grouptd").children().filter(".up_05").remove();
		$("#option_tr").css("display","none");
		document.getElementById("addbutton").disabled=true;
		$('#issearch').html(sele);	
		$('#tdIndexType').html(search1);
	}else if('SELECT'==inputType){
		$("#option_tr").css("display","block");
		document.getElementById("addbutton").disabled=false;
		$('#issearch').html(sele);
	  ...
	  <!--主要内容-->
	    <div class="main_pag" id="list">
		  ...
		  <tr id="background_tr" style='display:none'>
  			...
  			<tr id="option_tr" <#if  entity?? && entity.entryWay=='SELECT'> <#else>style='display:none'</#if> >
  			  <td class="label">可选值列表：</td>
  			  <td  id="grouptd" style="width:200px;">
  			  <#if  entity?? && entity.entryWay=='SELECT'>
  			  		<input type="button" name="button" id="addbutton" value="添加一个列表项" onclick="addPNode();" />
	  			   	<#if inputValuess??&&inputValuess?size gt 0>
		  			    ...
		  			    <p class="up_05">
		  			 <span style='display: none;'>    英文:</span> <input type="text" name="textfield2" id="textfield2" class="input200" onchange="changeText(this)" value="${inputValue.optionalName!}"/>
		  			      <input type="hidden" name="optionalNames" id="optionalNames" class="input200" value="${inputValue.optionalId!},${inputValue.optionalName!}"/>
		  			<span style='display: none;'>   中文: <input type="text" name="textfield2" id="textfield2" class="input200" onchange="changeText(this)" value="${inputValue.optionalNameZh!}"/>
	  			    ...
	  			    <#else>
		  			    <p class="up_05">
		  			      <span style='display: none;'>  英文:</span> <input type="text" name="textfield2" id="textfield2" class="input200" onchange="changeText(this)"/>
		  			      <input type="hidden" name="optionalNames" id="optionalNames" class="input200" value=""/>
		  			<span style='display: none;'>    中文: <input type="text" name="textfield2" id="textfield2" class="input200" onchange="changeText(this)" value=""/>
