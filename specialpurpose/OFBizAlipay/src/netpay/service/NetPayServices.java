...
import netpay.config.NetPayConfig;
...
public class NetPayServices {
	...
	public static String NetPayApi(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<String> orderIdList = (List<String>) request.getAttribute("orderIdList");
			...
			for (String orderId : orderIdList) {
				...
				orderHeader.store();
				BigDecimal totalFee = orderHeader.getBigDecimal("grandTotal");// .toPlainString();
				feeAmount = feeAmount.add(totalFee);
				// String orderName = orderHeader.getString("orderName");
			}

			long longTotal = (long) (Double.valueOf(feeAmount.toPlainString()) * 100);
			...
			String priv1 = orderIdSB.toString(); // 原路返回的信息
			String getId = ""; // 网关
			chinapay.PrivateKey key1 = new chinapay.PrivateKey();

			boolean flag;
			flag = key1.buildKey(partner, 0, System.getProperty("ofbiz.home") + "/specialpurpose/ofbiztools/config/netPay/MerPrk1.key"); // 注意要用哪一个商户号签名就要用对应的key文件。
			if (flag == false) {
			...
			String CheckValue = s1.Sign(partner + externalId + totalFee + currencyType + TransDate + gatewayType + priv1);
	 ...
	 * @return 提交表单HTML文本
	 */
	public static String buildRequest(Map<String, Object> sParaTemp, String strMethod) {
		NetPayConfig netPayConfig = new NetPayConfig();
		// 待请求参数数组
		...
		sbHtml.append("<form id=\"netPay\" name=\"netPay\" action=\"" + netPayConfig.paymentUrl + "\" method=\"" + strMethod + "\">");
