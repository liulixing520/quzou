...
function subSearch(formName){
	var  searchType=document.getElementById('searchPosType').value;
	if(searchType!=null&&searchType.length>0){
		if(searchType=='orderId'){
			document.getElementById('orderIdPos').value=document.getElementById('searchPosContent').value;
		}else if(searchType=='userLoginId'){
			document.getElementById('userLoginIdPos').value=document.getElementById('searchPosContent').value;
		}
...
<div id='pageContent'  class="pageContent" >
	<div class="panelBar">
		<ul class="toolBar">
		<form action='FindOrderPos' id='searchPosOrders' name='searchPosOrders' method="post" rel="pagerForm">
			  ...
			  <input type='hidden' name='orderId'  id='orderIdPos' />
			  <input type='hidden' name='userLoginId' id='userLoginIdPos' value='${requestParameters.userLoginId?if_exists}'/>
			...
			<li><select name='searchType' id='searchPosType'><option value='orderId'>订单号</option><option value='userLoginId'>下单人</option></select></li>
			<li><input type='text' name='searchContent' id='searchPosContent' onblur="if(this.value=='') this.value=''" onfocus="if(this.value=='') this.value=''" value="" ></li>
			<li><a class="search" href="#"  onclick="javascript:subSearch('searchPosOrders');" ><span>搜索</span></a></li>
			<li><a class="search" href="<@ofbizUrl>FindPosOrderAdvanced</@ofbizUrl>" target="dialog" mask="true" title="查询框"><span>高级检索</span></a></li>
		</form>	
			</ul>
	</div>
       <table class="table" width="100%" layoutH="78">
          <thead>
            ...
            <td width="13%" >
            <a href="/iteamgr/control/FindOrderPos?noConditionFind=Y&amp;sortField=orderDate" class="sort-order">下单时间</a>
            </td>
    ...
    <!--dwz  分页 -->      
	<@dwzPagin orderList orderListSize VIEW_SIZE_1 VIEW_INDEX_1  "FindOrderPos" "navTab"/>
</div>