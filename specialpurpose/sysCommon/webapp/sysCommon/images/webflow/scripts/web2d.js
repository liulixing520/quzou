...
function IconObject()
{
	...
	// 图元中的文字对象
	this.text = document.createElement("<input type='text' readonly='true'  style='position:absolute;background:transparent;border:10;font-size:9pt;text-align:center'>");
...
//				ClickedIconObject = window;
	...
	*/
	function _moveText() {
		this.text.style.left = this.img.offsetLeft + (HarfSizeOfIcon - LengthOfChar(this.displayName) * 5);
		this.text.style.top = this.img.offsetTop + HarfSizeOfIcon * 2 + 2;
		this.text.style.width = LengthOfChar(this.displayName) * 10;
	}
	//--------------_moveText()定义结束。-----------------------
...
function LineObject() 
{
	...
	*/
	function _drawText() {
		...
		this.text.style.left = sumX - LengthOfChar(this.displayName) * 5;
