 ...
 */
public class MultiShoppingListEvents {

    public static final String module = MultiShoppingListEvents.class.getName();
            ...
            if (cartIdInt != null) {
                ...
                if (allowPromo || !item.getIsPromo()) {
                  ...
                  //    ctx.put("accommodationSpotId", item.getAccommodationSpotId());
                        ...
                        ctx.put("modifiedPrice", item.getDisplayPrice());
                        ctx.put("shoppingCartHistoryId", delegator.getNextSeqId("ShoppingCartHistory"));
                        serviceResult = dispatcher.runSync("createShoppingListItem", ctx);
                        
                        //
                        serviceResult = dispatcher.runSync("createShoppingCartHistory", ctx);
                    } catch (GenericServiceException e) {
    ...
    /**
     * Finds or creates a specialized (auto-save) productStoreId List
     * @param delegator
     * @param dispatcher
     * @param partyId
     * @param userLogin
     ...
     */
    public static List<String> getAutoSaveProductStoreIdList(Delegator delegator, LocalDispatcher dispatcher, String partyId, GenericValue userLogin) throws GenericEntityException, GenericServiceException {
        ...
        if (partyId != null) {
            ...
            if (UtilValidate.isNotEmpty(existingLists)) {
            	...
            	for(GenericValue shoppingList:existingLists){
            		if(!shoppingList.getString("productStoreId").equals(productStoreId)){
            			productStoreIdList.add(shoppingList.getString("productStoreId"));
            		}
            		productStoreId = shoppingList.getString("productStoreId");
            	}
            }
        }


        return productStoreIdList;
    }
    ...
    public static void fillAutoSaveList(ShoppingCart cart, LocalDispatcher dispatcher) throws GeneralException {
        if (cart != null && dispatcher != null) {
            GenericValue userLogin = MultiShoppingListEvents.getCartUserLogin(cart);
    ...
    public static String restoreAutoSaveList(HttpServletRequest request, HttpServletResponse response) {
        ...
        List<String> productStoreIdList = null;
        
        try {
			productStoreIdList = getAutoSaveProductStoreIdList(delegator,dispatcher,null,userLogin);
        ...
        if(UtilValidate.isNotEmpty(productStoreIdList)){
        	for(String productStoreId:productStoreIdList){
		        ...
		        ShoppingCart cart = MultiShoppingCartEvents.getCartObject(request,null,null,productStoreId);
    ...
    public static String createGuestShoppingListCookies (HttpServletRequest request, HttpServletResponse response){
        ...
        if (UtilValidate.isEmpty(autoSaveListId)) {
        	try {
                Map<String, Object> listFields = UtilMisc.<String, Object>toMap("userLogin", userLogin, "shoppingListGroupName", PERSISTANT_LIST_NAME);
                Map<String, Object> newListResult = dispatcher.runSync("createShoppingListGroup", listFields);
                if (newListResult != null) {
                    autoSaveListId = (String) newListResult.get("shoppingListGroupId");
                }
            } catch (GeneralException e) {
                Debug.logError(e, module);
            }
        ...
        if (UtilValidate.isNotEmpty(autoSaveListId)) {
            if (UtilValidate.isNotEmpty(shoppingCartMap)) {
            	shoppingCartMap.setAutoSaveListId(autoSaveListId);
            } else {
