...
context.thruDate = nowTime.toString().substring(0,10);
dayBeginCal = Calendar.getInstance();
dayBeginCal.setTime(nowTime);
dayBeginCal.add(Calendar.DAY_OF_MONTH, -30);
dayBegin = new Timestamp(dayBeginCal.getTimeInMillis());
context.fromDate = dayBegin.toString().substring(0,10);
selectType = (String)parameters.get("selectType")?:"no";
context.selectType = selectType;
if("ok".equals(selectType)){
	...
	if(primaryProductCategoryId){
		...
		context.primaryProductCategoryId = primaryProductCategoryId;
	}
	...
	if(productTypeCategoryId){
		...
		context.productTypeCategoryId = productTypeCategoryId;
	}
	...
	for(product in productList){
		...
		fromDateStr = (String)parameters.get("fromDate");//?:dayBegin.toString().substring(0,10);
		thruDateStr = (String)parameters.get("thruDate");//?:nowTime.toString().substring(0,10);
