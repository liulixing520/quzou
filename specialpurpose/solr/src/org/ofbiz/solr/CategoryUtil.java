...
public abstract class CategoryUtil {
    ...
    public static List<List<String>> getStoreCategoryTrail(String productCategoryId, DispatchContext dctx) {
        GenericDelegator delegator = (GenericDelegator) dctx.getDelegator();
         List<List<String>> trailElements = FastList.newInstance();
         String parentProductCategoryId = productCategoryId;
         while (UtilValidate.isNotEmpty(parentProductCategoryId)) {
             ...
             try {
                 ...
                 rolllupConds.add(EntityCondition.makeCondition("productCategoryId", parentProductCategoryId));
                 rolllupConds.add(EntityUtil.getFilterByDateExpr());
                 List<GenericValue> productCategoryRollups = delegator.findList("ProductStoreCategoryRollup", EntityCondition.makeCondition(rolllupConds), null, UtilMisc.toList("-fromDate"), null, true);
                 if (UtilValidate.isNotEmpty(productCategoryRollups)) {
                     List<List<String>> trailElementsAux = FastList.newInstance();
                     trailElementsAux.addAll(trailElements);
                     // add only categories that belong to the top category to trail
                     for (GenericValue productCategoryRollup : productCategoryRollups) {
                         ...
                         sbf.append("productStoreId=").append(productCategoryRollup.getString("productStoreId")).append(";");
                         sbf.append("prodCatalogCategoryTypeId=").append(productCategoryRollup.getString("prodCatalogCategoryTypeId")).append(";");
                         sbf.append("productCategoryId=").append(trailCategoryId);
                         trailCategoryId = sbf.toString();
                         List<String> trailElement = FastList.newInstance();
                         if (!trailElements.isEmpty()) {
                             for (List<String> trailList : trailElementsAux) {
                                 trailElement.add(trailCategoryId);
