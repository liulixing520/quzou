...
public class UtilDateTime {
    ...
    public static java.sql.Date toSqlDate() {
    	java.util.Date newDate = nowDate();
	 ...
	 * @param c
	 */
	public static double checkDate(Date s, Date c) {
		Long time = s.getTime() - c.getTime();// 计算所得为微秒数
		double days = time / 1000 / 60 / 60 / 24;// 计算所得的天数
		return days;
	}
	 ...
	 * @param c
	 */
	public static List<String> getDateList(Date s, Date c) {
        ...
        while(start.compareTo(end) <= 0) {
            //打印每天
            String date1 = format.format(start.getTime());
