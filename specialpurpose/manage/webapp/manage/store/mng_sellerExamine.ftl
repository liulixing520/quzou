 ...
 <script type="text/javascript">
	function toCertificationPage(cer,partyId){
		if(cer=="SELLER_PERSONAL"){
			//window.open("<@ofbizUrl>stor_certification4</@ofbizUrl>?partyId="+partyId);
			window.location.href="<@ofbizUrl>stor_certification4</@ofbizUrl>?partyId="+partyId;
		}else if(cer=="SELLER_DOMECORP"){
			//window.open("<@ofbizUrl>stor_certification2</@ofbizUrl>?partyId="+partyId);
	    	window.location.href="<@ofbizUrl>stor_certification2</@ofbizUrl>?partyId="+partyId;
		}else if(cer=="SELLER_INDIVBIZ"){
			//window.open("<@ofbizUrl>stor_certification3</@ofbizUrl>?partyId="+partyId);
			window.location.href="<@ofbizUrl>stor_certification3</@ofbizUrl>?partyId="+partyId;
		}else{
			alert("该用户没有提交审核请求！");
		}
	}
...
<#if partyList?exists>
  ...
  <#if partyList?has_content>
    ...
    <#list partyList as partyRow>
      <#assign partyObj = delegator.findOne("Party",false,{'partyId':partyRow.partyId})>
      <#assign partyType = partyRow.getRelatedOne("PartyType", false)?if_exists>
      <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
        <td><#-- 
        <a href="<@ofbizUrl>viewprofile?partyId=${partyRow.partyId}</@ofbizUrl>">
        -->${partyRow.partyId}</td>
        <td>
        <#-- 
      ...
      <#else>
        ...
        <#if partyName?has_content>
          ${partyName} ${partyRow.partyTypeId!}
        <#else>
          (${uiLabelMap.PartyNoNameFound})
        </#if>
      </#if>
      -->
      	<#if partyRow.partyId?has_content>
      		<#if partyRow.partyTypeId?has_content && partyRow.partyTypeId=="PERSON">
      			<#assign person = delegator.findOne("Person", {"partyId":partyRow.partyId}, true)/>
      ...
      <#assign mainRole = dispatcher.runSync("getPartyMainRole", Static["org.ofbiz.base.util.UtilMisc"].toMap("partyId", partyRow.partyId, "userLogin", userLogin))/>
              ...
              -->
              ${partyObj.begDate?if_exists}
        </td>
        <td>
        	<#assign stores = delegator.findByAnd("ProductStoreRole",{"partyId":partyRow.partyId},null,false)/>
        	<#if stores?has_content>
        		<#assign storeRole = Static["org.ofbiz.entity.util.EntityUtil"].getFirst(stores)>
        		<#if storeRole?has_content>
        			<#assign store = delegator.findOne("ProductStore",Static["org.ofbiz.base.util.UtilMisc"].toMap("productStoreId", storeRole.productStoreId),false)/>
        			${store.storeName?if_exists}
        		</#if>
        	</#if>
        </td>
        <td>${partyObj.examineDate?if_exists}</td>
        <td class="button-col align-float">
        	<#if partyObj.sellerStatusId?has_content>
        		<#if partyObj.sellerStatusId=="SELLER_CREATED"><label>待审核</label></#if>
        		<#if partyObj.sellerStatusId=="SELLER_ACCEPTED"><label>通过</label></#if>
        		<#if partyObj.sellerStatusId=="SELLER_DECLINED"><label>不通过</label></#if>
        	<#else>
        		<label style="color:red;">无请求</label>
        	</#if>
        </td>
        ...
        <td class="button-col align-float">
          <#if partyObj.sellerStatusId?has_content>
          	...
          	<div style="display:Inline">
          		<#if partyObj.sellerStatusId=="SELLER_CREATED">
          			<a href="javascript:void(0);" onclick="setProductStoreStatus(this,'SELLER_ACCEPTED','${patyClassification[0].partyId}');">通过</a>&nbsp;|&nbsp;<a href="javascript:void(0);" onclick="setProductStoreStatus(this,'SELLER_DECLINED','${patyClassification[0].partyId}');">不通过</a>
          		</#if>
          		<#if partyObj.sellerStatusId=="SELLER_ACCEPTED" || partyObj.sellerStatusId=="SELLER_DECLINED">
          			<a href="javascript:void(0);" onclick="setProductStoreStatus(this,'SELLER_CREATED','${patyClassification[0].partyId}');">撤销</a>
          		</#if>
          	</div>
          </#if>
        </td>
