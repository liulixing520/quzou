...
if (productId) {
    ...
    if (product) {
        ...
        if("Y".equals(product.isVirtual) && "N".equals(product.isVariant)){
        	//获取抽象产品 的一个 实例类型子产品
            ...
            virtualVariantProductAssocs = delegator.findByAnd("ProductAssoc", ["productId": productId, "productAssocTypeId": "PRODUCT_VARIANT"], ["-fromDate"], false);
            ...
            context.add_product_id = null
        }else{
        	context.add_product_id = productId
        }
    }
...
org.ofbiz.product.product.ProductVisitWorker.saveProductVisit(request, productId);
