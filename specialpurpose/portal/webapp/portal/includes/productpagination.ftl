<#-- 分页调用 参数说明 -->
<#macro paginationSimple   listSize  viewSize nextviewIndex searchAction searchForm sortField>
<#if listSize&gt;0>
<#assign totalPage=Static["java.lang.Math"].ceil(listSize/viewSize)>
<#else>
<#assign totalPage=1>
</#if>
<#assign viewIndex=nextviewIndex+1>
<style type="text/css">
...
.m-page .items{float:left; list-style:none;}
...
.m-page .active,.m-page .active:hover{position:relative;z-index:1;background:#5bc0de;color:#fff;border-color:#5bc0de;cursor:default;text-decoration:none}
...
.m-page .text,.m-page .input,.m-page .btn{margin-left:5px; padding:0;}
        	...
        	<#--<div class="total">
	       	...
	       	<div class="form">
				...
				<a href="javascript:jump('${searchForm!}','${totalPage}');">
	         		<span tabindex="0"  class="btn J_Submit">${uiLabelMap.sellerDetermine}</span>
	         	</a>
	        </div>
	        -->
        </div>
...
function jump(searchFormId,pages){
		var ex = /^\d+$/;
		if (ex.test($("#inde").val())) {
			if($("#inde").val()>pages){
				return;
