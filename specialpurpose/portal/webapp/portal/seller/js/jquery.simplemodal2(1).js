...
(function($){
	...
	$.modal.impl={
		...
		bindEvents:function(){
			...
			//关闭按钮事件绑定
			var arrCloseBtn = this.dialog.container.find(".closebtn");	//class="closebtn"	
			var oThis = this;
		...
		redraw:function(){
			...
			var nScreenHeigth = window.screen.availHeight;//document.documentElement.offsetHeight || document.body.clientHeight;
		...
		getElementPositionInfo:function (strEleId){
			...
			function pageX(oEle){	
				if(oEle.parentElement.id=='modalContainer')return 0;
