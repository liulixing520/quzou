<#assign actionUrlStr="user_updatePartyEmailAddress">
<div class="screenlet">
<#--    <div class="screenlet-title-bar">
        <ul>
            <li class="h3">${uiLabelMap.PortalUserModifyMailbox}</li>
        </ul>
        <br class="clear">
    </div>-->
            ...
            <#if "EMAIL_ADDRESS" = contactMech.contactMechTypeId>
                <form method="post" action="<@ofbizUrl>${actionUrlStr}</@ofbizUrl>" name="editcontactmechform"
                    ...
                    <div class="button-bar">
                        <a href="javascript:document.editcontactmechform.submit()"
                           class="smallSubmit">${uiLabelMap.CommonSave}</a>
                    </div>
                </form>
            </#if>
        <#--<form name="partyDeleteContact" method="post" action="<@ofbizUrl>user_deleteContactMech</@ofbizUrl>" onsubmit="javascript:submitFormDisableSubmits(this)">
             <input name="partyId" value="${partyId}" type="hidden"/>
             <input name="contactMechId" value="${contactMech.contactMechId}" type="hidden"/>
